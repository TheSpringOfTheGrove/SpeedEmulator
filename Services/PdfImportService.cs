...
public sealed partial class PdfImportService : IPdfImportService
{
    ...
    [
        ...
        new("Time", 130, 173),
        new("Account", 173, 228),
        new("TransactionName", 228, 282),
        new("Amount", 282, 337),
        new("Balance", 337, 379),
        new("OppositeName", 379, 421),
        new("OppositeAccount", 421, 466),
        new("Summary", 466, 512)
    ];
    ...
    private static readonly PdfPositionedColumnSpec[] IcbcCorporateDebitCreditColumns =
    [
        new("Date", 35, 74),
        new("Time", 74, 112),
        new("OwnAccount", 112, 180),
        new("OppositeName", 180, 280),
        new("OppositeAccount", 280, 360),
        new("OppositeBank", 360, 440),
        new("VoucherNum", 440, 485),
        new("Direction", 485, 515),
        new("Debit", 515, 575),
        new("Credit", 575, 625),
        new("Summary", 625, 690),
        new("Usage", 690, 750),
        ...
        PdfExtractedDocument document)
    {
        ...
        if (string.Equals(bank.Name, "工行对公", StringComparison.OrdinalIgnoreCase))
        {
            if (GetIcbcCorporateLayout(signatureText) != IcbcCorporateLayout.Unsupported)
            {
                return;
            }
        ...
        if (string.Equals(bank.Name, "兴业对公", StringComparison.OrdinalIgnoreCase))
        {
            if (IsSupportedCibCorporateLayout(document))
            {
                return;
            }
    ...
    private static bool IsSupportedCibCorporateLayout(PdfExtractedDocument document)
    {
        return document.Lines.Any(line => Regex.IsMatch(
            CleanPdfValue(line.Text),
            @"^交易时间\s+摘要\s+凭证代号\s+支出\s+收入\s+收支方向\s+账户余额\s+对方账号\s+对方户名$"));
    }

    private static IcbcCorporateLayout GetIcbcCorporateLayout(string signatureText)
    {
        var common = new[] { "中国工商银行账户明细清单", "账号", "币种", "本方账号户名", "本方账号开户行", "时间范围" };
        if (!common.All(item => signatureText.Contains(item, StringComparison.Ordinal)))
        {
            return IcbcCorporateLayout.Unsupported;
        }

        if (new[] { "交易时间", "本方账号", "对方户名", "对方账号", "对方账户开户行", "凭证号", "借/贷", "借方发生额", "贷方发生额", "摘要", "用途", "余额" }
            .All(item => signatureText.Contains(item, StringComparison.Ordinal)))
        {
            return IcbcCorporateLayout.DebitCredit;
        }

        return IcbcCorporateLayout.Unsupported;
    }

    private enum IcbcCorporateLayout
    {
        Unsupported,
        DebitCredit
    }
            ...
            foreach (var page in document.GetPages())
            {
                ...
                try
                {
                    ...
                    foreach (var letter in page.Letters)
                    {
                        ...
                        extracted.Characters.Add(new PdfTextWord(
                            page.Number,
                            letter.Value,
                            bounds.Left,
                            bounds.Right,
    ...
    private static bool ParseCmbcPdf(Bank bank, BankUser user, PdfExtractedDocument document, PdfImportResult result)
    {
        ...
        foreach (var line in lines.TakeLast(160))
        {
            var printMatch = Regex.Match(
                CleanPdfValue(line.Text),
                @"(?:打印渠道[：:]\s*(?<channel>.*?)\s+)?打印柜员[：:]\s*(?<operator>\S+)\s+打印时间[：:]\s*(?<time>\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})");
            if (printMatch.Success)
            {
                SetUserNamed(user, bank, "打印日期", printMatch.Groups["time"].Value);
                SetUserNamed(user, bank, "活期一本通", printMatch.Groups["operator"].Value);
                break;
            }
        }
    ...
    private static bool ParseSpdbPdf(Bank bank, BankUser user, PdfExtractedDocument document, PdfImportResult result)
    {
        ...
        var positionedRows = BuildPositionedRowsWithTopLead(
            document.Words,
            SpdbPersonalColumns,
            IsSpdbPersonalHeaderWord,
            IsSpdbPersonalFooterWord,
            @"^\d{8}$",
            130,
            203,
            2);
        ...
        {
            if (TryParseSpdbPositionedRecord(row, bank, user, out var record))
            {
                result.FlowRecords.Add(record);
            }
        }
    ...
    private static bool ParseCibPdf(Bank bank, BankUser user, PdfExtractedDocument document, PdfImportResult result)
    {
        ...
        foreach (var line in lines)
        {
            ...
            var nameMatch = Regex.Match(text, @"^户\s*名\s*[:：]\s*(?<name>.+)$");
            if (nameMatch.Success)
            {
                SetUserNamed(user, bank, "姓名", nameMatch.Groups["name"].Value);
                parsedUser = true;
            ...
            var accountMatch = Regex.Match(text, @"^账\s*号\s*[:：]\s*(?<account>.+)$");
            if (accountMatch.Success)
            {
                SetUserNamed(user, bank, "卡号", accountMatch.Groups["account"].Value);
                parsedUser = true;
            ...
            var currencyMatch = Regex.Match(text, @"^币\s*种\s*[:：]\s*(?<currency>.+)$");
            if (currencyMatch.Success)
            {
                SetUserNamed(user, bank, "币种", currencyMatch.Groups["currency"].Value);
                parsedUser = true;
            ...
            var typeMatch = Regex.Match(text, @"^账户类型\s*[:：]\s*(?<type>.+)$");
            ...
            var printMatch = Regex.Match(text, @"^打印日期\s*[:：]\s*(?<print>\d{4}-\d{2}-\d{2}\s+[\d.:]+)$");
            if (printMatch.Success)
            {
    ...
    private static bool IsSpdbPersonalHeaderWord(PdfTextWord word)
    {
        return word.Text is "交易日期" or "交易时间" or "交易账号" or "交易名称"
            or "交易金额" or "账户余额" or "对手姓名" or "对手账号" or "交易摘要"
            or "Date" or "Time" or "Account" or "Name" or "Amount" or "Balance"
            or "Counter" or "Opponent" or "Summary";
    }

    private static bool IsSpdbPersonalFooterWord(string text)
    {
        var value = CleanPdfValue(text);
        return Regex.IsMatch(value, @"^第\s*\d+\s*页")
            || value.StartsWith("Page", StringComparison.OrdinalIgnoreCase);
    }

    private static bool TryParseSpdbPositionedRecord(
        ...
        out FlowRecord record)
    {
        ...
        record.Remark = GetPositionedCell(row, "Summary");
    ...
    private static bool ParseSpdbCorporatePdf(Bank bank, BankUser user, PdfExtractedDocument document, PdfImportResult result)
    {
        ...
        foreach (var line in lines.Take(180))
        {
            ...
            var customerNumberMatch = Regex.Match(
                text,
                @"客户(?:号|编号)\s+Customer\s+Number\s*[:：]?\s*(?<customer>[A-Za-z0-9]+)");
            if (customerNumberMatch.Success)
            {
                var customerNumber = CleanPdfValue(customerNumberMatch.Groups["customer"].Value);
                SetUserNamed(user, bank, "客户编号", customerNumber);
                user["客户号"] = customerNumber;
                parsedUser = true;
            }
    ...
    private static bool ParseIcbcCorporatePdf(Bank bank, BankUser user, PdfExtractedDocument document, PdfImportResult result)
    {
        ...
        foreach (var row in BuildPositionedRowsWithTopLead(
            document.Words,
            IcbcCorporateDebitCreditColumns,
            IsIcbcCorporateDebitCreditHeaderWord,
            ...
            12))
        {
            if (TryParseIcbcCorporateDebitCreditRecord(row, bank, user, out var record))
            {
                result.FlowRecords.Add(record);
            }
        }
    ...
    private static bool ParseBocomCorporatePdf(Bank bank, BankUser user, PdfExtractedDocument document, PdfImportResult result)
    {
        ...
        foreach (var row in BuildPositionedRowsWithTopLead(
            BuildBocomCorporatePositionedWords(document),
            ...
            126,
            10,
            joinCellWords: JoinBocomCorporateCharacterWords))
        {
        ...
        NormalizeBocomCorporateWrappedCells(result.FlowRecords);

        if (user.OpeningBalance == 0 && result.FlowRecords.FirstOrDefault() is { Balance: { } firstBalance } firstRecord)
        {
            var openingBalance = firstBalance
                + (firstRecord.DebitAmount ?? 0)
                - (firstRecord.CreditAmount ?? 0);
            user.OpeningBalance = (decimal)openingBalance;
            SetUserNamed(user, bank, "期初余额", openingBalance.ToString("0.00", CultureInfo.InvariantCulture));
        }
    ...
    private static bool ParseCmbcCorporatePdf(Bank bank, BankUser user, PdfExtractedDocument document, PdfImportResult result)
    {
        ...
        foreach (var line in lines.Take(90))
        {
            ...
            var printMatch = Regex.Match(
                text,
                @"打印渠道[：:]\s*(?<channel>.*?)\s+打印柜员[：:]\s*(?<operator>.*?)\s+打印时间[：:]\s*(?<print>\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})");
            if (printMatch.Success)
            {
                SetUserNamed(user, bank, "打印日期", printMatch.Groups["print"].Value);
                SetUserNamed(user, bank, "打印柜员", printMatch.Groups["operator"].Value);
                SetUserNamed(user, bank, "打印渠道", printMatch.Groups["channel"].Value);
                parsedUser = true;
            }
    ...
    private static bool TryParseIcbcCorporateDebitCreditRecord(
        PdfPositionedRow row,
        Bank bank,
        BankUser user,
        out FlowRecord record)
    {
        ...
        var ownAccount = GetPositionedCell(row, "OwnAccount");
        ...
        record.BankId = bank.Id;
        record.BankUserId = user.Id;
        record.Account = FirstNotBlank(ownAccount, user.AccountNo, user.CardNo);
        record.AccountTime = ParseDateTimeOrNull($"{FormatCompactDate(date)} {time}");
        record.ProductType = summary;
        record.ProductName = summary;
        record.ProductBrief = summary;
        record.Usage = usage;
        ...
        SetFlowRaw(record, "日期", $"{FormatCompactDate(date)} {time}");
        SetFlowRaw(record, "交易时间", $"{FormatCompactDate(date)} {time}");
        SetFlowRaw(record, "本方账号", ownAccount);
        SetFlowRaw(record, "借贷标志", direction);
        ...
        SetFlowRaw(record, "对方户名", oppositeName);
        SetFlowRaw(record, "对方账号", oppositeAccount);
        SetFlowRaw(record, "对方账户开户行", oppositeBank);
        return true;
    }
        ...
        out FlowRecord record)
    {
        ...
        var tradeName = GetPositionedCell(row, "TradeName")
            .Replace("二代支付来账自动入账", "二代支付来账-自动入账", StringComparison.Ordinal);
        var summary = GetPositionedCell(row, "Summary");
        var serial = GetPositionedCell(row, "Serial");
        var serialToken = Regex.Match(serial, @"[A-Za-z]{3}[A-Za-z0-9]+", RegexOptions.CultureInvariant);
        if (serialToken.Success)
        {
            var spill = string.Concat(serial.AsSpan(0, serialToken.Index), serial.AsSpan(serialToken.Index + serialToken.Length));
            summary = string.Concat(summary, spill);
            serial = serialToken.Value;
        }
        record.BankId = bank.Id;
        record.BankUserId = user.Id;
        record.Account = FirstNotBlank(user.AccountNo, user.CardNo);
    ...
    private static bool IsIcbcCorporateDebitCreditHeaderWord(PdfTextWord word)
    {
        return word.Text is "交易时间" or "本方账号" or "对方户名" or "对方账号" or "对方账户开户行"
            or "凭证号" or "借/贷" or "借方发生额" or "贷方发生额" or "摘要" or "用途" or "余额";
    }
    ...
    {
        return text.StartsWith("当前账单", StringComparison.Ordinal)
            || text.StartsWith("本页累计", StringComparison.Ordinal)
            || text.StartsWith("本月累计", StringComparison.Ordinal)
            || text.StartsWith("出单截止日期", StringComparison.Ordinal)
            || text.StartsWith("本月第", StringComparison.Ordinal)
            || text.StartsWith("第", StringComparison.Ordinal)
            || text.StartsWith("交通银行重庆市分行", StringComparison.Ordinal);
    }

    private static IReadOnlyList<PdfTextWord> BuildBocomCorporatePositionedWords(PdfExtractedDocument document)
    {
        var positioned = document.Words
            .Where(word => word.Top <= 130
                || IsBocomCorporateFooterWord(CleanPdfValue(word.Text))
                || GetHorizontalCenter(word) < 516
                || GetHorizontalCenter(word) >= 832)
            .Select(word => word with { Text = CleanPdfValue(word.Text) })
            .ToList();
        positioned.AddRange(document.Characters
            .Where(character => character.Top > 130
                && GetHorizontalCenter(character) >= 516
                && GetHorizontalCenter(character) < 832
                && !char.IsWhiteSpace(character.Text.FirstOrDefault())));
        return positioned;
    }

    private static string JoinBocomCorporateCharacterWords(IEnumerable<PdfTextWord> words)
    {
        var remaining = words.OrderBy(word => word.Top).ThenBy(word => word.Left).ToList();
        var lines = new List<List<PdfTextWord>>();
        foreach (var word in remaining)
        {
            var line = lines.LastOrDefault(items => Math.Abs(items[0].Top - word.Top) <= 2.5);
            if (line is null)
            {
                lines.Add([word]);
            }
        ...
        return CleanPdfValue(string.Concat(lines.SelectMany(line => line.OrderBy(word => word.Left)).Select(word => word.Text)));
    }
    ...
    private static bool IsCmbcCorporateFooterWord(string text)
    {
        return text.StartsWith("打印渠道:", StringComparison.Ordinal)
            || text.StartsWith("打印渠道：", StringComparison.Ordinal)
    ...
    private static void NormalizeBocomCorporateWrappedCells(IReadOnlyList<FlowRecord> records)
    {
        for (var index = 0; index < records.Count; index++)
        {
            var current = records[index];
            if (current.OppositeUsername.StartsWith("份有限公司", StringComparison.Ordinal))
            {
                var sameRowCarry = Regex.Match(
                    current.OppositeBank,
                    @"^(?<carry>.+股)(?<bank>[\p{IsCJKUnifiedIdeographs}]{2,12}银行.*)$");
                if (sameRowCarry.Success)
                {
                    current.OppositeUsername = string.Concat(sameRowCarry.Groups["carry"].Value, current.OppositeUsername);
                    current.OppositeBank = sameRowCarry.Groups["bank"].Value;
                }
            }

            if (index == 0 || !current.OppositeUsername.StartsWith("份有限公司", StringComparison.Ordinal))
            {
                UpdateBocomCorporateCounterpartyRaw(current);
                continue;
            }

            var previous = records[index - 1];
            var nameCarry = Regex.Match(
                previous.OppositeUsername,
                @"^(?<name>.*(?:经营部|有限公司|公司|商行|中心|门市|门店|店))(?<carry>.+股)$");
            if (nameCarry.Success)
            {
                previous.OppositeUsername = nameCarry.Groups["name"].Value;
                current.OppositeUsername = string.Concat(nameCarry.Groups["carry"].Value, current.OppositeUsername);
                UpdateBocomCorporateCounterpartyRaw(previous);
            }
    ...
    private static void UpdateBocomCorporateCounterpartyRaw(FlowRecord record)
    {
        SetFlowRaw(record, "对方户名", record.OppositeUsername);
        SetFlowRaw(record, "对方行名", record.OppositeBank);
    }
        ...
        out FlowRecord record)
    {
        ...
        {
            var accountAndName = Regex.Match(
                token,
                @"^(?<account>\d+)(?<name>[\p{IsCJKUnifiedIdeographs}].*)$");
            if (accountAndName.Success)
            {
                accountParts.Add(accountAndName.Groups["account"].Value);
                nameParts.Add(accountAndName.Groups["name"].Value);
            }
            else if (Regex.IsMatch(token, @"^\d+$") && nameParts.Count == 0)
            {
        ...
        foreach (var line in lines.Skip(2))
        {
            var accountAndName = Regex.Match(
                line,
                @"^(?<account>\d+)(?:\s+|(?=[\p{IsCJKUnifiedIdeographs}]))(?<name>[\p{IsCJKUnifiedIdeographs}].*)$");
            if (accountAndName.Success)
            {
                accountParts.Add(accountAndName.Groups["account"].Value);
                nameParts.Add(accountAndName.Groups["name"].Value);
        ...
        out FlowRecord record)
    {
        record = new FlowRecord();
        var text = JoinGroupText(group);
        var headMatch = Regex.Match(
            text,
            @"^(?<date>\d{4}-\d{2}-\d{2})\s+(?<accounting>\d{8}|\d{4}-\d{2}-\d{2})\s+(?<after>.+)$");
        if (!headMatch.Success)
        {
            headMatch = Regex.Match(
                text,
                @"^(?<date>\d{4}-\d{2}-\d{2})\s+(?<time>.*?)\s+(?<accounting>\d{8})\s+(?<after>.+)$");
        }
        if (!headMatch.Success)
        {
            return false;
        ...
        if (timeDigits.Length is > 0 and < 6)
        {
            return false;
        }

        var time = timeDigits.Length >= 6
            ? $"{timeDigits[..2]}:{timeDigits.Substring(2, 2)}:{timeDigits.Substring(4, 2)}"
            : string.Empty;
        var detail = headMatch.Groups["after"].Value;
        var detailMatch = Regex.Match(detail, @"^(?<summary>.+?)\s+(?<direction>[支收])\s+(?<amount>[+-]?\d[\d,]*\.\d{2})\s+(?<balance>[+-]?\d[\d,]*\.\d{2}|\*{3,})(?:\s+(?<tail>.*))?$");
        ...
        var balanceText = detailMatch.Groups["balance"].Value;
        var isMaskedBalance = balanceText.Contains('*', StringComparison.Ordinal);
        var tail = detailMatch.Groups["tail"].Value;
        var summary = CollapseChineseSeparatedWords(detailMatch.Groups["summary"].Value);
        var (usage, oppositeName, oppositeAccount, oppositeBank) = isMaskedBalance
            ? (string.Empty, string.Empty, string.Empty, string.Empty)
            : SplitCibTail(tail, summary);

        record.BankId = bank.Id;
        record.BankUserId = user.Id;
        record.Account = FirstNotBlank(user.AccountNo, user.CardNo);
        record.AccountTime = ParseDateTimeOrNull(string.IsNullOrWhiteSpace(time)
            ? headMatch.Groups["date"].Value
            : $"{headMatch.Groups["date"].Value} {time}");
        record.ProductName = summary;
        record.ProductBrief = summary;
        record.IncomeAttribute = direction == "支" ? "支出" : "收入";
        record.TradeMoney = signedAmount;
        record.Balance = isMaskedBalance ? null : ParseDoubleOrNull(balanceText);
        record.Usage = usage;
        record.Remark = usage;
        ...
        if (isMaskedBalance)
        {
            record.TradePlace = CollapseChineseSeparatedWords(tail);
        }
        ApplySignedAmountColumns(record, signedAmount);

        SetFlowRaw(record, "交易日期", record.AccountTime?.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) ?? headMatch.Groups["date"].Value);
        ...
        SetFlowRaw(record, "交易后余额", balanceText);
        SetFlowRaw(record, "对方户名", record.OppositeUsername);
        SetFlowRaw(record, "对方账号", record.OppositeAccount);
    ...
    private static IReadOnlyList<IReadOnlyList<PdfTextLine>> GroupCibRecords(IReadOnlyList<PdfTextLine> lines)
    {
        ...
        {
            var normalizedLines = pageLines
                .Select(line => new PdfTextLine(
                    line.PageNumber,
                    line.LineNumber,
                    CleanPdfValue(line.Text)))
                .ToList();
            result.AddRange(GroupLinesByStart(normalizedLines, IsCibRecordStart, IsCibIgnoredLine));
        }
    ...
    private static bool IsCibPageFooterLine(string text)
    {
        ...
        return value.StartsWith("\u4ea4\u6613\u7c7b\u578b\uff1a", StringComparison.Ordinal)
            ...
            || Regex.IsMatch(value, @"^(?:\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s*)+$")
    ...
    private static (string Usage, string OppositeName, string OppositeAccount, string OppositeBank) SplitCibTail(
        string tail,
        string summary)
    {
        ...
        if (accountIndex < 0)
        {
            // Counter-withdrawal/deposit rows have no counterparty account. Their
            // final two cells are still explicit: counterparty name = 现金 and
            // counterparty bank = 兴业银行. Do not merge them into transaction usage.
            if (tokens.Count >= 2
                && tokens[^1].Contains("银行", StringComparison.Ordinal)
                && (summary.Contains("柜员", StringComparison.Ordinal)
                    || summary.Contains("ATM", StringComparison.OrdinalIgnoreCase)))
            {
                return (
                    string.Empty,
                    CollapseChineseSeparatedWords(string.Join(' ', tokens.Take(tokens.Count - 1))),
                    string.Empty,
                    CollapseChineseSeparatedWords(tokens[^1]));
            }

            return (CollapseChineseSeparatedWords(string.Join(' ', tokens)), string.Empty, string.Empty, string.Empty);
        }

        var beforeAccountTokens = tokens.Take(accountIndex).ToList();
        ...
        var (usage, name) = SplitCibUsageAndName(beforeAccountTokens);
        return (usage, name, account, bank);
    }

    private static (string Usage, string Name) SplitCibUsageAndName(IReadOnlyList<string> tokens)
    {
        var cleanedTokens = tokens
            .Select(CleanPdfValue)
            .Where(item => !string.IsNullOrWhiteSpace(item))
            .ToList();
        var combined = CollapseChineseSeparatedWords(string.Join(' ', cleanedTokens));
        var knownSplit = SplitCibUsageAndName(combined);
        if (!string.IsNullOrWhiteSpace(knownSplit.Usage))
        {
            return knownSplit;
        }

        if (cleanedTokens.Count >= 2)
        {
            var firstToken = CollapseChineseSeparatedWords(cleanedTokens[0]);
            if (IsKnownCibUsage(firstToken))
            {
                return (
                    firstToken,
                    CollapseChineseSeparatedWords(string.Join(' ', cleanedTokens.Skip(1))));
            }

            // Long company names are frequently wrapped exactly at the boundary
            // between Usage and Counterparty Name. When the first fragment is not
            // a genuine usage value, the complete pre-account text is the name.
    ...
    private static bool IsKnownCibUsage(string value)
    {
        var text = CollapseChineseSeparatedWords(value);
        return text is "货款" or "转账" or "借款" or "还款" or "工资" or "现金"
            || text.Contains("二维码付款", StringComparison.Ordinal)
            || text.Contains("红包", StringComparison.Ordinal)
    ...
    private static (string Usage, string Name) SplitCibUsageAndName(string value)
    {
        ...
        // Some Industrial Bank rows contain the same value in the adjacent
        // "Transaction Usage" and "Counterparty's Account Name" columns.
        // PDF text extraction places both cells consecutively, and collapsing
        // the cell whitespace makes them look like one duplicated name.
        if (text.Length % 2 == 0)
        {
            var halfLength = text.Length / 2;
            var firstHalf = text[..halfLength];
            if (string.Equals(firstHalf, text[halfLength..], StringComparison.Ordinal))
            {
                var repeatedValue = CleanPdfValue(firstHalf);
                return (repeatedValue, repeatedValue);
            }
        }
    ...
    private static void FillMissingPdfRecordTimes(IList<FlowRecord> records, Bank bank)
    {
        var sourceDates = records
            ...
            .ToList();
        var firstDifferentDate = sourceDates
            .Skip(1)
            .FirstOrDefault(date => date != sourceDates.FirstOrDefault());
        var sourceIsDescending = firstDifferentDate != default
            ? sourceDates[0] > firstDifferentDate
        ...
        foreach (var group in groups)
        {
            ...
            var seconds = CreateSortedRandomSeconds(bank.Name, group.Key, items.Count, sourceIsDescending);
    ...
    private sealed class PdfExtractedDocument
    {
        ...
        public List<PdfTextWord> Characters { get; } = [];
