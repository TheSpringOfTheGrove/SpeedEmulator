...
public sealed class FlowAutoGenerator
{
    ...
    private const string AmountPrecisionField = "__GeneratedAmountPrecision";
    ...
    private const string AmountDistributionKeyField = "__GeneratedAmountDistributionKey";
    ...
    private static readonly ConcurrentDictionary<Type, RulePropertyMapping[]> RulePropertyMappings = new();
    private static readonly ConcurrentDictionary<AmountBounds, double[]> RuleAmountPoolValues = new();
    private static readonly ConcurrentDictionary<IReadOnlyList<ColumnDefinition>, NormalizedColumnDefinition[]> NormalizedColumnDefinitions = new();
    private static readonly ConcurrentDictionary<(IReadOnlyList<ColumnDefinition> Rule, IReadOnlyList<ColumnDefinition> Flow), ColumnFieldMapping[]> ColumnFieldMappings = new();
    private static readonly ConcurrentDictionary<string, string> NormalizedColumnAliases = new(StringComparer.Ordinal);
    ...
    {
        NormalizedColumnDefinitions.Clear();
        ColumnFieldMappings.Clear();
        RuleRecordTemplates.Clear();
        BankFlowFieldPresence.Clear();
        RecordAmountBounds.Clear();
        ...
        var estimatedRecordCapacity = IsWechatBank(request.Bank)
            ? Math.Max(512, ((end.Date - start.Date).Days + 1) * 24)
            : 512;
        var records = new List<FlowRecord>(estimatedRecordCapacity);
        ...
        if (!IsWechatBank(request.Bank) && request.References.Count < 200)
        {
            ...
            perfTrace.Mark("base early coverage", records);
        }
        GenerateNativeOptionalReferenceRecords(request, monthlyPlan, targetIncome, random, records, scheduleState, amountUsage);
        perfTrace.Mark("base optional", records);
        EnsureNativeOptionalReferenceRuleCoverage(request, monthlyPlan, targetIncome, random, records, scheduleState, amountUsage);
        perfTrace.Mark("base coverage", records);
        CompleteNativeConfiguredTotals(request, monthlyPlan, start, end, targetIncome, random, records, scheduleState, amountUsage);
        perfTrace.Mark("base complete totals", records);
        ExpandWechatReferenceVolume(request, start, end, random, records, scheduleState, amountUsage);
        perfTrace.Mark("base wechat volume", records);
        ...
        perfTrace.Mark("final balance waves", records);
        ForceNativeNonNegativeBalances(records, request, openingBalance, start, end, random);
        perfTrace.Mark("final balance nonnegative", records);
        RestoreFinalBalancePreservingNonNegative(records, request, openingBalance);
        perfTrace.Mark("final balance restore 1", records);
        if (!highVolumeAmountPostProcessing)
        {
            NormalizeSmallDiscreteAmountDistribution(records, request, random);
            NormalizeReferenceAmountDistribution(records, random);
            perfTrace.Mark("final balance normalize", records);
            var finalAmountChanged = ReducePositiveFinalBalanceSafely(records, request, openingBalance);
            finalAmountChanged = AddSafeExpenseRecordsForPositiveFinalBalance(
                records,
                request,
                start,
                end,
                openingBalance,
                random) || finalAmountChanged;
            finalAmountChanged = ReducePositiveFinalBalanceSafely(records, request, openingBalance)
                || finalAmountChanged;
            perfTrace.Mark("final balance expense", records);
            finalAmountChanged = ForceNativeNonNegativeBalances(
                records,
                request,
                openingBalance,
                start,
            ...
            perfTrace.Mark("final balance restore 2", records);
        }
        else
        {
            ReducePositiveFinalBalanceSafely(records, request, openingBalance);
        ...
        perfTrace.Mark("final balance income restore", records);
        ...
        RestoreConfiguredIncomeWithMatchedExpense(records, request, start, end, openingBalance, random);
        EnforceActualMonthlyIncomeCap(records, request, monthlyPlan, openingBalance, random);
        EnforceMonthlyClosingBalanceTargets(records, request, monthlyPlan, openingBalance, random);
        ApplyBalances(records, openingBalance);
        ...
        CompactWechatReferenceVolume(request, start, end, random, records);
        ApplyBalances(records, openingBalance);
        if (GetMinimumBalance(records, openingBalance) < -0.009d)
        {
        ...
        EnsureWideExpenseUpperSegmentPresence(records, openingBalance, random);
        ApplyBalances(records, openingBalance);
        perfTrace.Mark("monthly closing prepare", records);
        if (GetMinimumBalance(records, openingBalance) < -0.009d)
        {
            RepairVeryHighVolumeNegativeBalances(records, openingBalance);
            ApplyBalances(records, openingBalance);
        }
        perfTrace.Mark("monthly closing nonnegative", records);
        EnforceMonthlyClosingBalanceCapByMovingFutureExpenses(
            records,
            request,
            monthlyPlan,
            openingBalance,
            random);
        perfTrace.Mark("monthly closing cap", records);
        RestoreFinalMonthlyClosingTarget(records, request, monthlyPlan, openingBalance, random);
        if (EnforceActualMonthlyIncomeCap(records, request, monthlyPlan, openingBalance, random)
            && GetMinimumBalance(records, openingBalance) < -0.009d)
        {
            RepairVeryHighVolumeNegativeBalances(records, openingBalance);
        }
    ...
    private static bool EnsureWideExpenseUpperSegmentPresence(
        List<FlowRecord> records,
        double openingBalance,
        Random random)
    {
        if (records.Count == 0 || GetMinimumBalance(records, openingBalance) < -0.009d)
        {
            return false;
        }

        var amountSnapshot = CaptureRecordAmountSnapshot(records);
        var expenseTotalBefore = SumExpense(records);
        var changed = false;
        var groups = records
            .Where(IsReferenceGeneratedRecord)
            .Where(item => item.TradeMoney < -0.009d && !IsSystemInterestRecord(item))
            .GroupBy(item =>
            {
            ...
            .Where(group => group.Key.Max >= 100_000d - 0.009d)
            .Where(group => group.Count() >= 3)
            .ToList();

        foreach (var group in groups)
        {
            var bounds = new AmountBounds(group.Key.Min, group.Key.Max, group.Key.Unit, group.Key.Precision);
            ...
            var tailSegments = segments
                .Skip(Math.Max(0, segments.Count - 4))
                .ToList();
            var tailMinimum = tailSegments[0].Min;
            var groupRecords = group
                .Where(item => item.AccountTime.HasValue)
                .OrderBy(item => item.AccountTime)
                .ToList();
            var desiredUpperCount = Math.Max(
                1,
                (int)Math.Round(groupRecords.Count * 0.16d, MidpointRounding.AwayFromZero));
            var existingUpperCount = groupRecords.Count(item =>
                Math.Abs(item.TradeMoney ?? 0) >= tailMinimum - 0.009d);
            var missingUpperCount = Math.Max(0, desiredUpperCount - existingUpperCount);
            if (missingUpperCount <= 0)
            {
                continue;
            }

            var targets = groupRecords
                .Where(item => Math.Abs(item.TradeMoney ?? 0) < tailMinimum - 0.009d)
                ...
                .SelectMany(monthGroup => monthGroup
                    ...
                    .Take(Math.Max(1, (int)Math.Ceiling(missingUpperCount / (double)Math.Max(1, groupRecords
                        .Select(record => new DateTime(record.AccountTime!.Value.Year, record.AccountTime.Value.Month, 1))
                        .Distinct()
                        .Count())))))
                .Take(missingUpperCount)
                .ToList();
            var segmentOffset = random.Next(tailSegments.Count);
            for (var targetIndex = 0; targetIndex < targets.Count; targetIndex++)
            {
                var target = targets[targetIndex];
                var targetMonth = new DateTime(
                    target.AccountTime!.Value.Year,
                    target.AccountTime.Value.Month,
                    1);
                var donors = groupRecords
                    .Where(item => !ReferenceEquals(item, target))
                    .Where(item => item.AccountTime < target.AccountTime)
                    .Where(item => item.AccountTime.HasValue
                        && item.AccountTime.Value.Year == targetMonth.Year
                        && item.AccountTime.Value.Month == targetMonth.Month)
                    .Where(item => Math.Abs(item.TradeMoney ?? 0) < tailMinimum - 0.009d)
                    .ToList();
                var available = RoundMoney(donors.Sum(item =>
                    Math.Max(0, Math.Abs(item.TradeMoney ?? 0) - bounds.Min)));
                var current = RoundMoney(Math.Abs(target.TradeMoney ?? 0));
                var targetSegment = tailSegments[(segmentOffset + targetIndex) % tailSegments.Count];
                var ratio = 0.18d + (random.NextDouble() * 0.64d);
                var desired = ClampAmountToBounds(
                    targetSegment.Min + ((targetSegment.Max - targetSegment.Min) * ratio),
                    bounds);
                desired = Math.Min(desired, ClampAmountToBounds(current + available, bounds));
                if (desired < targetSegment.Min - 0.009d || desired <= current + 0.009d)
                {
                    continue;
                }

                var snapshot = CaptureRecordAmountSnapshot(groupRecords);
                var transfer = RoundMoney(desired - current);
                if (!TryDecreaseExpenseDonorsBalanced(donors, bounds, transfer, random))
                {
                    RestoreRecordAmounts(snapshot);
                    continue;
                }

                ApplySignedAmount(target, -1d, desired);
                changed = true;
            }
        }

        if (Math.Abs(SumExpense(records) - expenseTotalBefore) > 0.009d)
        {
    ...
    private static bool TryDecreaseExpenseDonorsBalanced(
        IReadOnlyList<FlowRecord> donors,
        AmountBounds bounds,
        double amount,
        Random random)
    {
        var unit = Math.Max(0.01d, bounds.Unit);
        var remaining = RoundAmountToUnit(Math.Max(0, amount), unit);
        for (var pass = 0; pass < 4 && remaining >= unit - 0.009d; pass++)
        {
            var candidates = donors
                .Where(item => Math.Abs(item.TradeMoney ?? 0) >= bounds.Min + unit - 0.009d)
                .OrderByDescending(item => Math.Abs(item.TradeMoney ?? 0))
                .ThenBy(_ => random.Next())
                .ToList();
            for (var index = 0; index < candidates.Count && remaining >= unit - 0.009d; index++)
            {
                var record = candidates[index];
                var current = RoundMoney(Math.Abs(record.TradeMoney ?? 0));
                var room = RoundMoney(current - bounds.Min);
                var slotsLeft = candidates.Count - index;
                var requested = RoundAmountToUnit(remaining / Math.Max(1, slotsLeft), unit);
                var reduction = FloorAmountToUnit(Math.Min(room, Math.Min(remaining, Math.Max(unit, requested))), unit);
                if (reduction < unit - 0.009d)
                {
                    continue;
                }

                var nextAmount = ClampAmountToBounds(current - reduction, bounds);
                var actualReduction = RoundMoney(current - nextAmount);
                if (actualReduction <= 0.009d || actualReduction > remaining + 0.009d)
                {
                    var fallback = FindValidAmountAtOrAbove(
                        Math.Max(bounds.Min, current - remaining),
                        current - unit,
                        bounds);
                    ...
                    nextAmount = ClampAmountToBounds(fallback.Value, bounds);
                    actualReduction = RoundMoney(current - nextAmount);
                    if (actualReduction <= 0.009d || actualReduction > remaining + 0.009d)
                    {
                        continue;
                    }
    ...
    private static bool RedistributeFinalMonthlyExpenseWaves(
        List<FlowRecord> records,
        FlowAutoGenerationRequest request,
        DateTime start,
        DateTime end,
    ...
    {
        var months = EnumerateMonths(start, end).ToList();
        if (months.Count < 4 || records.Count == 0)
        {
            return false;
        }

        ApplyBalances(records, openingBalance);
        var averageIncome = Math.Max(1d, SumIncome(records) / months.Count);
        var scheduleState = NativeScheduleState.From(records);
        var dayCounts = BuildDayCounts(records, start, end);
        var changed = false;
        var cycleStart = 0;
        var totalMoves = 0;
        var maxTotalMoves = Math.Min(600, Math.Max(80, records.Count / 2));

        while (cycleStart < months.Count - 1 && totalMoves < maxTotalMoves)
        {
            var buildLength = random.Next(1, 4);
            var drainIndex = Math.Min(months.Count - 1, cycleStart + buildLength);
            var drainMonth = months[drainIndex];
            var drainDays = Math.Max(1, (drainMonth.End.Date - drainMonth.Start.Date).Days + 1);
            var drainWindowStart = drainMonth.Start.Date.AddDays(Math.Max(0, (int)Math.Floor(drainDays * 0.45d)));
            var lowTarget = Math.Max(100d, averageIncome * (0.001d + (random.NextDouble() * 0.005d)));
            var closingBalance = CalculateBalanceThroughDate(records, openingBalance, drainMonth.End);
            var movesThisCycle = 0;

            while (closingBalance > lowTarget + 0.009d
                   && totalMoves < maxTotalMoves
                   && movesThisCycle < 100)
            {
                var spendable = RoundMoney(Math.Max(0, closingBalance - 0.01d));
                var desired = RoundMoney(Math.Max(0, closingBalance - lowTarget));
                var candidates = records
                    .Where(item => (item.TradeMoney ?? 0) < -0.009d)
                    .Where(item => item.AccountTime.HasValue && item.AccountTime.Value > drainMonth.End)
                    ...
                    .Where(item => item.Amount <= spendable + 0.009d)
                    .Where(item => scheduleState.GetCandidateDates(drainWindowStart, drainMonth.End, item.Rule!).Count > 0)
                    .OrderBy(item => Math.Abs(desired - item.Amount))
                ...
                var originalTime = selected.Record.AccountTime!.Value;
                var movedTime = PickNativeDistributedTime(
                    ...
                    random,
                    scheduleState);
                selected.Record.AccountTime = movedTime;
                scheduleState.Move(selected.Record, originalTime);
                dayCounts[originalTime.Date] = Math.Max(0, dayCounts.GetValueOrDefault(originalTime.Date) - 1);
                dayCounts[movedTime.Date] = dayCounts.GetValueOrDefault(movedTime.Date) + 1;
                ApplyBalances(records, openingBalance);
                if (GetMinimumBalance(records, openingBalance) < -0.009d)
                {
                    selected.Record.AccountTime = originalTime;
                    scheduleState.Move(selected.Record, movedTime);
                    dayCounts[movedTime.Date] = Math.Max(0, dayCounts.GetValueOrDefault(movedTime.Date) - 1);
                    dayCounts[originalTime.Date] = dayCounts.GetValueOrDefault(originalTime.Date) + 1;
                    ApplyBalances(records, openingBalance);
                    break;
                }

                closingBalance = CalculateBalanceThroughDate(records, openingBalance, drainMonth.End);
                changed = true;
                movesThisCycle++;
                totalMoves++;
            }

            cycleStart = drainIndex + 1;
        }

        return changed;
    }

    private static bool EnforceMonthlyClosingBalanceTargets(
        List<FlowRecord> records,
        FlowAutoGenerationRequest request,
    ...
    {
        if (records.Count == 0
            || monthlyPlan.Months.Count == 0
            || monthlyPlan.ClosingBalanceTargets.Length == 0)
        {
            return false;
        }
        ...
        for (var pass = 0; pass < 2; pass++)
        {
            ...
            for (var monthIndex = 0;
                 monthIndex < monthlyPlan.Months.Count
                 && monthIndex < monthlyPlan.ClosingBalanceTargets.Length;
                 monthIndex++)
            {
                ...
                {
                    var monthExpenses = GetAdjustableMonthlyExpenseRecords(records, month.Start, month.End);
                    var increased = IncreaseMonthlyExpenseRecordsSafely(
                        records,
                        monthExpenses,
                        openingBalance,
                        difference);
                    var remaining = RoundMoney(difference - Math.Max(0, increased));
                    if (remaining > MonthlyClosingBalanceTolerance)
                    {
                        var added = AddMonthlyClosingExpenseRecords(
                            records,
                            request,
                            month.Start,
                            month.End,
                            ...
                            remaining,
                            random);
                        remaining = RoundMoney(remaining - added);
                    }

                    passChanged = passChanged || remaining < difference - 0.009d;
                }
                else if (difference < -MonthlyClosingBalanceTolerance)
                {
                    passChanged = ReduceMonthlyOptionalExpense(
                        records,
                        month.Start,
                        month.End,
                        -difference) || passChanged;
                }
        ...
        ApplyBalances(records, openingBalance);
        DeferOptionalExpensesToRepairNegativeBalances(records, request, openingBalance, random);
        ApplyBalances(records, openingBalance);
        return changed;
    }

    private static double IncreaseMonthlyExpenseRecordsSafely(
        List<FlowRecord> records,
        IEnumerable<FlowRecord> candidates,
        double openingBalance,
        double amount)
    {
        var remaining = RoundMoney(Math.Max(0, amount));
                     ...
                     .ToList())
        {
            ...
            var current = RoundMoney(Math.Abs(record.TradeMoney ?? 0));
            var bounds = GetRecordAmountBounds(record);
            var ruleRoom = RoundMoney(bounds.Max - current);
            var safeRoom = RoundMoney(Math.Max(
                0,
                balanceIndex.GetSuffixMinimum(record, openingBalance) - 0.01d));
            var increase = Math.Min(remaining, Math.Min(ruleRoom, safeRoom));
            increase = FloorAmountToUnit(increase, Math.Max(0.01d, bounds.Unit));
            if (increase <= 0.009d)
            {
                continue;
            }

            var nextAmount = ClampAmountToBounds(current + increase, bounds);
            var applied = RoundMoney(nextAmount - current);
            if (applied <= 0.009d)
            {
                continue;
            }
            ...
            ApplySignedAmount(record, -1d, nextAmount);
            balanceIndex.AddToSuffix(record, -applied);
            increased = RoundMoney(increased + applied);
            remaining = RoundMoney(amount - increased);
        }

        return increased;
    }

    private static bool RestoreFinalMonthlyClosingTarget(
        List<FlowRecord> records,
        FlowAutoGenerationRequest request,
        MonthlyAmountPlan monthlyPlan,
        double openingBalance,
        Random random)
    {
        ...
        ApplyBalances(records, openingBalance);
        var month = monthlyPlan.Months[^1];
        var target = RoundMoney(monthlyPlan.ClosingBalanceTargets[^1]);
        var actual = CalculateBalanceThroughDate(records, openingBalance, month.End);
        var difference = RoundMoney(actual - target);
        if (difference > MonthlyClosingBalanceTolerance)
        {
            var candidates = GetAdjustableMonthlyExpenseRecords(records, month.Start, month.End);
            var increased = IncreaseMonthlyExpenseRecordsSafely(records, candidates, openingBalance, difference);
            var remaining = RoundMoney(difference - increased);
            if (remaining > MonthlyClosingBalanceTolerance)
            {
                increased = RoundMoney(increased + AddMonthlyClosingExpenseRecords(
                    records,
                    request,
                    month.Start,
                    month.End,
            ...
            ApplyBalances(records, openingBalance);
            return increased > 0.009d;
        }

        if (difference < -MonthlyClosingBalanceTolerance)
        {
            var changed = false;
            var remaining = RoundMoney(-difference);
            for (var monthIndex = monthlyPlan.Months.Count - 1;
                 monthIndex >= 0 && remaining > MonthlyClosingBalanceTolerance;
                 monthIndex--)
            {
                ApplyBalances(records, openingBalance);
                ...
                var finalBefore = CalculateBalanceThroughDate(records, openingBalance, month.End);
                var candidateMonth = monthlyPlan.Months[monthIndex];
                if (!ReduceMonthlyOptionalExpense(
                        records,
                        candidateMonth.Start,
                        candidateMonth.End,
                        adjustment))
                {
                    continue;
                ...
                PruneZeroAmountRecords(records);
                ApplyBalances(records, openingBalance);
                var finalAfter = CalculateBalanceThroughDate(records, openingBalance, month.End);
                var applied = RoundMoney(Math.Max(0, finalAfter - finalBefore));
                remaining = RoundMoney(remaining - applied);
                changed = changed || applied > 0.009d;
            }
    ...
    private static bool EnforceMonthlyClosingBalanceCapByMovingFutureExpenses(
        List<FlowRecord> records,
        FlowAutoGenerationRequest request,
        MonthlyAmountPlan monthlyPlan,
        double openingBalance,
        Random random)
    {
        ...
        var moveSequence = 0;
        var directMoves = 0;
        var expenseTransfers = 0;
        var incomeTransfers = 0;
        var loopCount = 0;
        var maxMoves = Math.Min(2400, Math.Max(240, records.Count * 2));
        var usedTimes = records
            .Where(item => item.AccountTime.HasValue)
            .Select(item => TrimToSecond(item.AccountTime!.Value))
            .ToHashSet();
        for (var monthIndex = 0;
             monthIndex < monthlyPlan.Months.Count - 1 && moveSequence < maxMoves;
             monthIndex++)
        {
            var month = monthlyPlan.Months[monthIndex];
            ApplyBalances(records, openingBalance);
            ...
            {
                var batchBalanceIndex = BalanceRangeIndex.Create(records, openingBalance);
                var batchMoves = MoveFutureExpenseRecordsToMonthBatch(
                    records,
                    request,
                    month.Start,
                    month.End,
                    ...
                    random,
                    moveSequence,
                    maxMoves - moveSequence,
                    batchBalanceIndex);
                if (batchMoves > 0)
                {
                    changed = true;
                    moveSequence += batchMoves;
                    directMoves += batchMoves;
                    ApplyBalances(records, openingBalance);
                }
            }

            while (moveSequence < maxMoves)
            {
                loopCount++;
                ApplyBalances(records, openingBalance);
                var balanceIndex = BalanceRangeIndex.Create(records, openingBalance);
                ...
                var candidates = records
                    .Where(item => item.AccountTime.HasValue && item.AccountTime.Value > month.End)
                    .Where(item => (item.TradeMoney ?? 0) < -0.009d)
                    .Where(IsGeneratedReferenceRecord)
                    .Where(item => !IsRequiredGeneratedRecord(item) && !IsSystemInterestRecord(item))
                    .Select(item => new
                    ...
                    .ThenBy(item => Math.Abs(excess - item.Amount))
                    .Take(80)
                    .ToList();
                var moved = false;
                foreach (var candidate in candidates)
                {
                    if (candidate.Amount > closing - 0.01d)
                    {
                        continue;
                    }
                    ...
                    var movedTime = PickLatestRuleTimeAfter(
                        candidate.Rule!,
                        month.Start.AddTicks(-1),
                        month.End,
                        usedTimes,
                        random,
                        moveSequence);
                    if (!movedTime.HasValue)
                    {
                        usedTimes.Add(TrimToSecond(originalTime));
                        continue;
                    }

                    var balanceBeforeMove = balanceIndex.GetBalanceBefore(movedTime.Value, openingBalance);
                    var intervalMinimum = balanceIndex.GetMinimum(
                        movedTime.Value,
                        originalTime,
                        balanceBeforeMove);
                    var safeRoom = RoundMoney(Math.Min(balanceBeforeMove, intervalMinimum) - 0.01d);
                    if (balanceIndex.GlobalMinimum >= -0.009d
                        && candidate.Amount <= safeRoom + 0.009d)
                    {
                        candidate.Record.AccountTime = movedTime.Value;
                        usedTimes.Add(TrimToSecond(movedTime.Value));
                        changed = true;
                        moved = true;
                        moveSequence++;
                        directMoves++;
                ...
                if (!moved)
                {
                    moved = TransferFutureExpenseAmountToMonth(
                        records,
                        request,
                        month.Start,
                        month.End,
                    ...
                    moved = TransferCurrentIncomeAmountToFutureMonth(
                        records,
                        month.Start,
                        month.End,
                        openingBalance,
        ...
        if (FlowGenerationPerfTrace.IsEnabled)
        {
            Console.Error.WriteLine(
                string.Create(
                    CultureInfo.InvariantCulture,
                    $"[FlowPerf] monthly closing moves: loops={loopCount}, total={moveSequence}, direct={directMoves}, expense-transfer={expenseTransfers}, income-transfer={incomeTransfers}"));
        }
    ...
    private static int MoveFutureExpenseRecordsToMonthBatch(
        ...
        double targetAmount,
        HashSet<DateTime> usedTimes,
        Random random,
        int sequence,
        int moveLimit,
        BalanceRangeIndex balanceIndex)
    {
        if (targetAmount <= MonthlyClosingBalanceTolerance || moveLimit <= 0)
        {
            return 0;
        }

        var candidates = records
            .Where(item => item.AccountTime.HasValue && item.AccountTime.Value > monthEnd)
            .Where(item => (item.TradeMoney ?? 0) < -0.009d)
            .Where(IsGeneratedReferenceRecord)
            .Where(item => !IsRequiredGeneratedRecord(item) && !IsSystemInterestRecord(item))
            .Select(item => new
            {
                ...
                Rule = ResolveRecordSourceRule(request, item) as GenerateReferenceRule,
                Amount = RoundMoney(Math.Abs(item.TradeMoney ?? 0)),
                OriginalTime = item.AccountTime!.Value
            })
            .Where(item => item.Rule is not null)
            .OrderBy(item => Math.Abs(targetAmount - item.Amount))
            .ThenByDescending(item => item.Amount)
            .ThenBy(item => item.OriginalTime)
            .ToList();
        var sameGapExpenses = new Dictionary<int, List<(DateTime Time, double Amount)>>();
        var remaining = targetAmount;
        var movedCount = 0;
        foreach (var candidate in candidates)
        {
            if (remaining <= MonthlyClosingBalanceTolerance || movedCount >= moveLimit)
            {
                break;
            }

            if (candidate.Amount > closingBalance - 0.01d)
            {
                continue;
            }

            usedTimes.Remove(TrimToSecond(candidate.OriginalTime));
            var movedTime = PickLatestRuleTimeAfter(
                candidate.Rule!,
            ...
            if (!movedTime.HasValue)
            {
                usedTimes.Add(TrimToSecond(candidate.OriginalTime));
                continue;
            }

            var insertionIndex = balanceIndex.GetInsertionIndex(movedTime.Value);
            var balanceBeforeMove = balanceIndex.GetBalanceBefore(movedTime.Value, openingBalance);
            if (sameGapExpenses.TryGetValue(insertionIndex, out var gapExpenses))
            {
                balanceBeforeMove = RoundMoney(balanceBeforeMove - gapExpenses
                    .Where(item => item.Time < movedTime.Value)
            ...
            var intervalMinimum = balanceIndex.GetMinimum(
                ...
                balanceBeforeMove);
            var safeRoom = RoundMoney(Math.Min(balanceBeforeMove, intervalMinimum) - 0.01d);
            if (balanceIndex.GlobalMinimum < -0.009d || candidate.Amount > safeRoom + 0.009d)
            {
                usedTimes.Add(TrimToSecond(candidate.OriginalTime));
                continue;
            }
            ...
            candidate.Record.AccountTime = movedTime.Value;
            usedTimes.Add(TrimToSecond(movedTime.Value));
            balanceIndex.AddToRange(movedTime.Value, candidate.OriginalTime, -candidate.Amount);
            if (!sameGapExpenses.TryGetValue(insertionIndex, out gapExpenses))
            {
                gapExpenses = [];
                sameGapExpenses[insertionIndex] = gapExpenses;
            }

            gapExpenses.Add((movedTime.Value, candidate.Amount));
            closingBalance = RoundMoney(closingBalance - candidate.Amount);
            remaining = RoundMoney(remaining - candidate.Amount);
            movedCount++;
        }
    ...
    private static bool TransferFutureExpenseAmountToMonth(
        List<FlowRecord> records,
        FlowAutoGenerationRequest request,
        DateTime monthStart,
        DateTime monthEnd,
        ...
        Random random,
        int sequence,
        BalanceRangeIndex? balanceIndex = null)
    {
        var receivers = records
            .Where(item => item.AccountTime.HasValue
                && item.AccountTime.Value >= monthStart
                && item.AccountTime.Value <= monthEnd)
            .Where(item => (item.TradeMoney ?? 0) < -0.009d)
            .Where(IsGeneratedReferenceRecord)
            .Where(item => !IsSystemInterestRecord(item))
            .Select(item => new
            ...
            .Where(item => item.Bounds.Max - item.Amount > 0.009d)
            .OrderByDescending(item => item.Record.AccountTime)
            .ThenByDescending(item => item.Bounds.Max - item.Amount)
            .Take(48)
            .ToList();
        var donors = records
            .Where(item => item.AccountTime.HasValue && item.AccountTime.Value > monthEnd)
            ...
            .Where(item => item.Rule is not null)
            .Where(item => item.Amount - item.Bounds.Min > 0.009d)
            .OrderBy(item => item.Record.AccountTime)
            .ThenByDescending(item => item.Amount - item.Bounds.Min)
            .Take(64)
            .ToList();
        balanceIndex ??= BalanceRangeIndex.Create(records, openingBalance);
        foreach (var donor in donors)
        {
            foreach (var receiver in receivers)
            {
                var transfer = Math.Min(
                    targetAmount,
                    Math.Min(
                        donor.Amount - donor.Bounds.Min,
                        receiver.Bounds.Max - receiver.Amount));
                if (TryTransferSameDirectionAmount(
                        records,
                        donor.Record,
                        receiver.Record,
                        transfer,
        ...
        foreach (var donor in donors)
        {
            if (IsRequiredGeneratedRecord(donor.Record))
            {
                continue;
            }

            var transferable = Math.Min(targetAmount, donor.Amount - donor.Bounds.Min);
            transferable = FloorAmountToUnit(transferable, Math.Max(0.01d, donor.Bounds.Unit));
            if (transferable < donor.Bounds.Min - 0.009d)
            {
                continue;
            }

            var accountTime = PickLatestRuleTimeAfter(
                donor.Rule!,
                monthStart.AddTicks(-1),
                monthEnd,
                usedTimes,
            ...
            var originalDonor = donor.Amount;
            var nextDonor = ClampAmountToBounds(originalDonor - transferable, donor.Bounds);
            var actualTransfer = RoundMoney(originalDonor - nextDonor);
            if (actualTransfer < donor.Bounds.Min - 0.009d)
            {
                continue;
            }

            ApplySignedAmount(donor.Record, -1d, nextDonor);
            var receiver = CreateRecordFromRule(
                request,
                donor.Rule!,
                request.Bank.ReferenceColumns,
                accountTime.Value,
                -actualTransfer);
            receiver.ExtraFields[RequiredOccurrenceField] = bool.FalseString;
            records.Add(receiver);
            ApplyBalances(records, openingBalance);
            if (GetMinimumBalance(records, openingBalance) >= -0.009d)
            {
                return true;
            ...
            ApplySignedAmount(donor.Record, -1d, originalDonor);
            ApplyBalances(records, openingBalance);
        }
    ...
    private static bool TransferCurrentIncomeAmountToFutureMonth(
        List<FlowRecord> records,
        DateTime monthStart,
        DateTime monthEnd,
        double openingBalance,
    ...
    private static bool TryTransferSameDirectionAmount(
        List<FlowRecord> records,
        FlowRecord donor,
        FlowRecord receiver,
        ...
        BalanceRangeIndex balanceIndex)
    {
        if (!donor.AccountTime.HasValue
            || !receiver.AccountTime.HasValue
            || donor.AccountTime.Value == receiver.AccountTime.Value)
        {
            return false;
        }

        var donorAmount = RoundMoney(Math.Abs(donor.TradeMoney ?? 0));
        var receiverAmount = RoundMoney(Math.Abs(receiver.TradeMoney ?? 0));
        var donorBounds = GetRecordAmountBounds(donor);
        var receiverBounds = GetRecordAmountBounds(receiver);
        var unit = Math.Max(0.01d, Math.Max(donorBounds.Unit, receiverBounds.Unit));
        var sign = (donor.TradeMoney ?? 0) < 0 ? -1d : 1d;
        if ((sign < 0 && receiver.AccountTime.Value >= donor.AccountTime.Value)
            || (sign > 0 && donor.AccountTime.Value >= receiver.AccountTime.Value))
        {
            return false;
        }

        var earlierTime = donor.AccountTime.Value < receiver.AccountTime.Value
            ? donor.AccountTime.Value
            : receiver.AccountTime.Value;
        var laterTime = donor.AccountTime.Value > receiver.AccountTime.Value
            ? donor.AccountTime.Value
            : receiver.AccountTime.Value;
        if (balanceIndex.GlobalMinimum < -0.009d)
        {
        ...
        var intervalMinimum = balanceIndex.GetMinimum(earlierTime, laterTime, openingBalance);
        var maximumSafeTransfer = RoundMoney(Math.Max(0, intervalMinimum - 0.01d));
        var transfer = FloorAmountToUnit(
            Math.Min(
                requestedAmount,
                Math.Min(
                    maximumSafeTransfer,
                    Math.Min(
                        donorAmount - donorBounds.Min,
                        receiverBounds.Max - receiverAmount))),
            unit);
        if (transfer <= 0.009d)
        {
            return false;
        ...
        for (var attempt = 0; attempt < 20 && transfer > 0.009d; attempt++)
        {
            var nextReceiver = ClampAmountToBounds(receiverAmount + transfer, receiverBounds);
            var receiverIncrease = RoundMoney(nextReceiver - receiverAmount);
            if (receiverIncrease <= 0.009d)
            {
                transfer = FloorAmountToUnit(transfer - unit, unit);
                continue;
            }

            var nextDonor = ClampAmountToBounds(donorAmount - receiverIncrease, donorBounds);
            var donorDecrease = RoundMoney(donorAmount - nextDonor);
            if (Math.Abs(receiverIncrease - donorDecrease) > 0.009d)
            {
                transfer = FloorAmountToUnit(Math.Min(receiverIncrease, donorDecrease) - unit, unit);
                continue;
            }

            ApplySignedAmount(donor, sign, nextDonor);
            ApplySignedAmount(receiver, sign, nextReceiver);
            return true;
        }
    ...
    private static double GetSafeExpenseIncreaseRoom(
        IReadOnlyList<FlowRecord> records,
        DateTime accountTime)
    {
        var minimum = records
            .Where(item => item.AccountTime.HasValue && item.AccountTime.Value >= accountTime)
            .Select(item => item.Balance ?? 0)
            .DefaultIfEmpty(0)
            .Min();
        return RoundMoney(Math.Max(0, minimum - 0.01d));
    }

    private static bool DeferOptionalExpensesToRepairNegativeBalances(
        List<FlowRecord> records,
        FlowAutoGenerationRequest request,
        double openingBalance,
        Random random)
    {
        ...
        var changed = false;
        var maxMoves = Math.Min(240, Math.Max(24, records.Count / 20));
        for (var move = 0; move < maxMoves; move++)
        {
            ApplyBalances(records, openingBalance);
            if (!TryFindFirstNegativeBalance(
                    records,
                    openingBalance,
                    out var firstNegativeTime,
                    out var minimumBalance)
                || minimumBalance >= -0.009d)
            {
                break;
            }

            var monthStart = new DateTime(firstNegativeTime.Year, firstNegativeTime.Month, 1);
            var monthEnd = MinDateTime(
                NormalizeEndDate(request.Config.EndTime),
            ...
            var usedTimes = records
                ...
                .ToHashSet();
            var moved = false;
            var futureIncomes = records
                .Where(item => item.AccountTime.HasValue
                    && item.AccountTime.Value > firstNegativeTime
                    && item.AccountTime.Value <= monthEnd)
                .Where(item => (item.TradeMoney ?? 0) > 0.009d)
                .Where(IsGeneratedReferenceRecord)
                .Where(item => !IsSystemInterestRecord(item))
                ...
                .Take(24)
                .ToList();
            foreach (var candidate in futureIncomes)
            {
                if (ResolveRecordSourceRule(request, candidate) is not GenerateReferenceRule rule)
                {
                    continue;
                ...
                var originalTime = candidate.AccountTime!.Value;
                usedTimes.Remove(TrimToSecond(originalTime));
                var advancedTime = PickLatestRuleTimeBefore(
                    rule,
                    monthStart,
                    firstNegativeTime,
                    usedTimes,
                    random,
                    move);
                if (!advancedTime.HasValue)
                {
                    usedTimes.Add(TrimToSecond(originalTime));
                    continue;
                }

                candidate.AccountTime = advancedTime.Value;
                ApplyBalances(records, openingBalance);
                var adjustedMinimum = GetMinimumBalance(records, openingBalance);
                if (adjustedMinimum > minimumBalance + 0.009d)
                {
                    changed = true;
                    moved = true;
                    break;
                ...
                candidate.AccountTime = originalTime;
                usedTimes.Add(TrimToSecond(originalTime));
                ApplyBalances(records, openingBalance);
            }
            ...
            foreach (var candidate in candidates)
            {
                ...
                var originalTime = candidate.AccountTime!.Value;
                usedTimes.Remove(TrimToSecond(originalTime));
                var deferredTime = PickLatestRuleTimeAfter(
                    rule,
                    firstNegativeTime,
                    monthEnd,
                    usedTimes,
                ...
                candidate.AccountTime = deferredTime.Value;
                ApplyBalances(records, openingBalance);
                var adjustedMinimum = GetMinimumBalance(records, openingBalance);
                if (adjustedMinimum > minimumBalance + 0.009d)
                {
    ...
    private static DateTime? PickLatestRuleTimeBefore(
        FlowRuleBase rule,
        DateTime start,
        DateTime before,
        IReadOnlySet<DateTime> usedTimes,
        Random random,
        int sequence)
    {
        ...
        for (var date = before.Date; date >= start.Date; date = date.AddDays(-1))
        {
            if (rule.TradeWeekend == false
                && date.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday)
            {
                continue;
            }

            var earliest = MaxDateTime(start, date.AddHours(startHour));
            var latest = date.AddHours(endHour).AddMinutes(59).AddSeconds(59);
            if (latest >= before)
            {
                latest = TrimToSecond(before.AddSeconds(-1));
            }
            ...
            {
                var candidate = latest.AddSeconds(-((sequence * 17 + offset * 11 + random.Next(0, 11)) % 120));
                if (candidate < earliest || candidate >= before)
                {
                    continue;
                }

                candidate = TrimToSecond(candidate);
    ...
    private static DateTime? PickLatestRuleTimeAfter(
        FlowRuleBase rule,
        DateTime after,
        DateTime end,
        IReadOnlySet<DateTime> usedTimes,
        ...
        int sequence)
    {
        ...
        for (var date = end.Date; date >= after.Date; date = date.AddDays(-1))
        {
            if (rule.TradeWeekend == false
                && date.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday)
            {
            ...
            if (latest > end)
            {
                latest = TrimToSecond(end);
            }

            var earliest = date.AddHours(startHour);
            if (latest < earliest || latest <= after)
            {
                continue;
            }
            ...
            for (var offset = 0; offset < 120; offset++)
            {
                var candidate = latest.AddSeconds(-((sequence * 13 + offset * 7 + random.Next(0, 7)) % 120));
                if (candidate < earliest || candidate <= after)
                {
                    continue;
                }
    ...
    private static List<FlowRecord> GetAdjustableMonthlyExpenseRecords(
        IEnumerable<FlowRecord> records,
        DateTime monthStart,
        DateTime monthEnd)
    {
    ...
    private static bool ReduceMonthlyOptionalExpense(
        List<FlowRecord> records,
        DateTime monthStart,
        DateTime monthEnd,
        double amount)
    {
        ...
        var changed = false;
        var candidates = GetAdjustableMonthlyExpenseRecords(records, monthStart, monthEnd)
            .OrderByDescending(item => Math.Abs(item.TradeMoney ?? 0))
            .ThenByDescending(item => item.AccountTime ?? DateTime.MinValue)
            .ToList();
        foreach (var record in candidates)
        {
            ...
            var absolute = RoundMoney(Math.Abs(record.TradeMoney ?? 0));
            var bounds = GetRecordAmountBounds(record);
            var partialAmount = RoundMoney(absolute - remaining);
            if (partialAmount >= bounds.Min - 0.009d)
            {
                var nextAmount = ClampAmountToBounds(partialAmount, bounds);
                var reduction = RoundMoney(absolute - nextAmount);
                if (reduction > 0.009d)
                {
                    ApplySignedAmount(record, -1d, nextAmount);
            ...
            if (absolute <= remaining + MonthlyClosingBalanceTolerance)
            {
                ApplySignedAmount(record, -1d, 0);
            ...
            ApplySignedAmount(record, -1d, bounds.Min);
            remaining = RoundMoney(remaining - reducible);
            changed = true;
        }
    ...
    private static double AddMonthlyClosingExpenseRecords(
        List<FlowRecord> records,
        FlowAutoGenerationRequest request,
        DateTime monthStart,
        DateTime monthEnd,
        double openingBalance,
        double targetClosingBalance,
        double amount,
        Random random)
    {
        var remaining = RoundMoney(Math.Max(0, amount));
        var candidates = request.References
            ...
            .ToList();
        var rules = SelectNativeAmountFillRules(candidates);
        if (remaining <= 0.009d || rules.Count == 0)
        {
            return 0;
        }
        ...
        var maxAttempts = Math.Clamp(
            ...
            rules.Count,
            IsWechatBank(request.Bank) ? 600 : 2400);
        ApplyBalances(records, openingBalance);
        var balanceIndex = BalanceRangeIndex.Create(records, openingBalance);
        var usedTimes = records
            .Where(item => item.AccountTime.HasValue)
            .Select(item => TrimToSecond(item.AccountTime!.Value))
            .ToHashSet();
        var closingBalance = CalculateBalanceThroughDate(records, openingBalance, monthEnd);
        for (var sequence = 0; sequence < maxAttempts && remaining > MonthlyClosingBalanceTolerance; sequence++)
        {
            var rulesWithTimes = rules
                .Select(rule => new
                {
                    Rule = rule,
                    Time = PickLatestRuleTimeAfter(
                        rule,
                        monthStart.AddTicks(-1),
                        monthEnd,
                        usedTimes,
                ...
                .Select(item => new
                {
                    ...
                    SafeRoom = RoundMoney(Math.Max(
                        0,
                        balanceIndex.GetSuffixMinimum(item.Time.Value, 0) - 0.01d))
                })
                .ToList();
            var usableRules = rulesWithTimes
                .Where(item => GetRuleAmountBounds(item.Rule).Min <= remaining + 0.009d)
                .Where(item => GetRuleAmountBounds(item.Rule).Min <= item.SafeRoom + 0.009d)
                .OrderByDescending(item => GetRuleAmountBounds(item.Rule).Max >= remaining ? 1 : 0)
                .ThenByDescending(item => GetRuleAmountBounds(item.Rule).Max)
                .ToList();
            var selected = usableRules.FirstOrDefault();
            if (selected is null)
            {
                break;
            }
            ...
            var bounds = GetRuleAmountBounds(rule);
            var upper = Math.Min(bounds.Max, Math.Min(remaining, selected.SafeRoom));
            if (upper < bounds.Min - 0.009d)
            {
                break;
            }

            var effectiveBounds = new AmountBounds(bounds.Min, upper, bounds.Unit, bounds.Precision);
            var amountToAdd = MoveAmountAwayFromEdges(upper, effectiveBounds, random);
            amountToAdd = ClampAmountToBounds(amountToAdd, effectiveBounds);
            if (amountToAdd <= 0.009d)
            {
                continue;
            }
            ...
            var record = CreateRecordFromRule(
                ...
                selected.Time,
                -amountToAdd);
            record.ExtraFields[RequiredOccurrenceField] = bool.FalseString;
            records.Add(record);
            usedTimes.Add(TrimToSecond(selected.Time));
            balanceIndex.AddToSuffix(selected.Time, -amountToAdd);
            added = RoundMoney(added + amountToAdd);
            remaining = RoundMoney(amount - added);

            closingBalance = RoundMoney(closingBalance - amountToAdd);
            if (closingBalance <= targetClosingBalance + MonthlyClosingBalanceTolerance)
            {
                break;
            }
        }
    ...
    private static double CalculateBalanceThroughDate(
        IEnumerable<FlowRecord> records,
        double openingBalance,
        DateTime end)
    {
        return RoundMoney(openingBalance + records
            .Where(item => item.AccountTime.HasValue && item.AccountTime.Value <= end)
            .Sum(item => item.TradeMoney ?? 0));
    }
    ...
    private static bool TryMoveIncomeToDate(
        ...
        IReadOnlyList<NativeMoveCandidate>? moveCandidates = null)
    {
        var monthlyIncomeCounts = BuildMonthlyIncomeCounts(records);
        var candidates = (moveCandidates ?? BuildNativeMoveCandidates(records, request))
            .Where(item => (item.Record.TradeMoney ?? 0) > 0.009d)
            ...
            .Where(item => item.SourceDate == targetDate || dayCounts.GetValueOrDefault(item.SourceDate) > 1)
            .Where(item => IsNativeDateAllowedForRule(targetDate, item.Rule))
            .Where(item => CanMoveIncomeRecordForRhythm(item.Record, monthlyIncomeCounts))
        ...
        NativeScheduleState? scheduleState = null)
    {
        ...
        var splitAmount = requestedSplitAmount.HasValue
            ? ClampAmountToBounds(requestedSplitAmount.Value, new AmountBounds(bounds.Min, Math.Min(bounds.Max, originalAmount - bounds.Min), bounds.Unit, bounds.Precision))
    ...
    private static Dictionary<int, int> BuildMonthlyIncomeCounts(IEnumerable<FlowRecord> records)
    {
    ...
    private static bool CanMoveIncomeRecordForRhythm(
        FlowRecord record,
        IReadOnlyDictionary<int, int> monthlyIncomeCounts)
    {
        if ((record.TradeMoney ?? 0) <= 0.009d || !record.AccountTime.HasValue)
        {
            return true;
        ...
        var month = record.AccountTime.Value;
        var sameMonthCount = monthlyIncomeCounts.GetValueOrDefault((month.Year * 100) + month.Month);
    ...
    private sealed class MonthlyAmountPlan
    {
        ...
        public required double MonthlyIncomeCap { get; init; }
    }
    ...
    private sealed class NativeScheduleState
    {
        ...
        public NativeScheduleState(int expectedRecordCount = 0)
        {
            var expectedDayCount = Math.Min(2048, Math.Max(0, expectedRecordCount));
            totalByDay = new Dictionary<DateTime, int>(expectedDayCount);
            incomeByDay = new Dictionary<DateTime, int>(expectedDayCount);
            expenseByDay = new Dictionary<DateTime, int>(expectedDayCount);
            usedTimes = new HashSet<DateTime>(Math.Max(0, expectedRecordCount));
            candidateDatesByRange = new Dictionary<string, DateTime[]>();
        }
        ...
        {
            var expectedRecordCount = records.TryGetNonEnumeratedCount(out var count) ? count : 0;
            var state = new NativeScheduleState(expectedRecordCount);
            foreach (var record in records)
            {
                state.Register(record);
            }
    ...
    private sealed class AmountUsageTracker
    {
        ...
        private readonly Dictionary<string, Dictionary<int, int>> tailUsageByGroup = new(StringComparer.Ordinal);
        private readonly Dictionary<string, Dictionary<int, int>> globalTailUsage = new(StringComparer.Ordinal);
        ...
        public void Register(FlowRecord record)
        {
            ...
            var precision = GetRecordAmountBounds(record).Precision;
            if (precision is not > 0)
            {
                return;
            }

            var tail = GetConfiguredPrecisionTail(amount, precision.Value);
            if (!tailUsageByGroup.TryGetValue(groupKey, out var tailUsage))
            {
                tailUsage = [];
                tailUsageByGroup[groupKey] = tailUsage;
            }

            tailUsage[tail] = tailUsage.GetValueOrDefault(tail) + 1;
            var globalKey = CreateGlobalTailKey(sign, precision.Value);
            if (!globalTailUsage.TryGetValue(globalKey, out var globalUsage))
            {
                globalUsage = [];
                globalTailUsage[globalKey] = globalUsage;
            }

            globalUsage[tail] = globalUsage.GetValueOrDefault(tail) + 1;
        }

        public IReadOnlyDictionary<double, int> GetUsage(
            string sourceKey,
            int sign,
            AmountBounds bounds,
            bool useFullSourceBounds)
        {
            if (!usageByGroup.TryGetValue(CreateGroupKey(sourceKey, sign), out var usage)
                || usage.Count == 0)
            {
                return EmptyAmountUsage;
            }

            if (useFullSourceBounds)
            {
                return usage;
            }
        ...
        public IReadOnlyDictionary<int, int> GetTailUsage(string sourceKey, int sign)
        {
            return tailUsageByGroup.TryGetValue(CreateGroupKey(sourceKey, sign), out var usage)
                ? usage
                : EmptyTailUsage;
        }

        public IReadOnlyDictionary<int, int> GetGlobalTailUsage(int sign, int precision)
        {
            return globalTailUsage.TryGetValue(CreateGlobalTailKey(sign, precision), out var usage)
                ? usage
                : EmptyTailUsage;
        }
        ...
        private static string CreateGlobalTailKey(int sign, int precision)
        {
            return string.Create(CultureInfo.InvariantCulture, $"{sign}:{precision}");
        }

        private static readonly IReadOnlyDictionary<int, int> EmptyTailUsage = new Dictionary<int, int>();
        private static readonly IReadOnlyDictionary<double, int> EmptyAmountUsage = new Dictionary<double, int>();
    }
        ...
        AmountUsageTracker amountUsage)
    {
        ...
        for (var monthIndex = 0; monthIndex < monthlyPlan.Months.Count; monthIndex++)
        {
            ...
            var throughMonth = records.Where(item =>
                item.AccountTime.HasValue && item.AccountTime.Value <= month.End);
            var availableThroughMonth = RoundMoney(Math.Max(
                0,
                SumIncome(throughMonth) - SumExpense(throughMonth)));
            var existingMonthExpense = SumExpense(records.Where(item =>
                IsRecordInRange(item, month.Start, month.End)));
            var expenseTarget = Math.Min(
                monthExpenseTarget,
                RoundMoney(existingMonthExpense + availableThroughMonth));
            GenerateNativeOptionalForMonth(
                request,
                month.Start,
                month.End,
    ...
    private static void FinalizeConfiguredPrecisionReferenceAmounts(
        List<FlowRecord> records,
        double openingBalance,
        Random random)
    {
        var perfTrace = FlowGenerationPerfTrace.Start();
        var precisionRecords = records
            .Where(IsGeneratedReferenceRecord)
            .Where(item => !IsSystemInterestRecord(item))
            .Where(item => Math.Abs(item.TradeMoney ?? 0) > 0.009d)
            .Where(item => GetRecordAmountBounds(item).Precision is > 0)
            .ToList();
        if (precisionRecords.Count < 2)
        {
            return;
        }

        var tailSnapshot = CaptureRecordAmountSnapshot(precisionRecords);
        BalanceConfiguredPrecisionTailUsage(precisionRecords, random, requireBalanceSafe: false);
        perfTrace.Mark("precision tail balance", precisionRecords);
        ApplyBalances(records, openingBalance);
        if (GetMinimumBalance(records, openingBalance) < -0.009d)
        {
            RestoreRecordAmounts(tailSnapshot);
            BalanceConfiguredPrecisionTailUsage(precisionRecords, random, requireBalanceSafe: true);
            ApplyBalances(records, openingBalance);
        }
        perfTrace.Mark("precision tail balance validation", precisionRecords);

        var timelineSnapshot = CaptureRecordAmountSnapshot(precisionRecords);
        if (SpreadConfiguredPrecisionAmountsAcrossTimeline(precisionRecords, random))
        {
            ApplyBalances(records, openingBalance);
            if (GetMinimumBalance(records, openingBalance) < -0.009d)
            {
                RestoreRecordAmounts(timelineSnapshot);
                ApplyBalances(records, openingBalance);
            }
        }
        perfTrace.Mark("precision timeline spread", precisionRecords);

        var spacingSnapshot = CaptureRecordAmountSnapshot(precisionRecords);
        ImproveConfiguredPrecisionTailSpacing(precisionRecords, random, requireBalanceSafe: false);
        perfTrace.Mark("precision tail spacing", precisionRecords);
        ApplyBalances(records, openingBalance);
        if (GetMinimumBalance(records, openingBalance) < -0.009d)
        {
            RestoreRecordAmounts(spacingSnapshot);
            ImproveConfiguredPrecisionTailSpacing(precisionRecords, random, requireBalanceSafe: true);
            ApplyBalances(records, openingBalance);
        }
        perfTrace.Mark("precision tail spacing validation", precisionRecords);
    }

    private static bool BalanceConfiguredPrecisionTailUsage(
        IReadOnlyList<FlowRecord> records,
        Random random,
        bool requireBalanceSafe)
    {
        var changed = false;
        var groups = records
            .GroupBy(item =>
            {
                ...
                return (
                    Sign: GetAmountSign(item.TradeMoney ?? 0),
                    Precision: Math.Clamp(bounds.Precision ?? 0, 1, 2));
            })
            .Where(group => group.Key.Sign != 0)
            .ToList();
        var protectedTailOwnerSign = groups
            .Where(group => group.Key.Precision == 2)
            .OrderByDescending(group => group.Count())
            .Select(group => group.Key.Sign)
            .FirstOrDefault();
        ...
        foreach (var group in groups)
        {
            var groupRecords = group
                ...
                .ThenBy(item => item.Index)
                .ToList();
            var precision = group.Key.Precision;
            var scale = precision == 1 ? 10 : 100;
            var validTails = Enumerable.Range(1, scale - 1)
                .Where(tail => tail % 10 != 0)
                .ToArray();
            var validTailLookup = new bool[scale];
            foreach (var tail in validTails)
            {
                validTailLookup[tail] = true;
            }

            var targetTailUsage = CreateConfiguredPrecisionTailTargets(
                groupRecords.Count,
                precision,
                validTails,
                allowProtectedTails: precision != 2 || group.Key.Sign == protectedTailOwnerSign);
            var targetUsage = new double[scale];
            var maximumUsage = new int[scale];
            foreach (var tail in validTails)
            {
                targetUsage[tail] = targetTailUsage.GetValueOrDefault(tail);
                maximumUsage[tail] = GetConfiguredPrecisionTailMaximumUsage(targetUsage[tail]);
            }

            var amounts = new double[groupRecords.Count];
            var bounds = new AmountBounds[groupRecords.Count];
            var tails = new int[groupRecords.Count];
            var usage = new int[scale];
            for (var index = 0; index < groupRecords.Count; index++)
            {
                amounts[index] = RoundMoney(Math.Abs(groupRecords[index].TradeMoney ?? 0));
                bounds[index] = GetRecordAmountBounds(groupRecords[index]);
                tails[index] = GetConfiguredPrecisionTail(amounts[index], precision);
                if (tails[index] >= 0 && tails[index] < scale)
                {
                    usage[tails[index]]++;
                }
            }

            var maxMoves = Math.Max(32, groupRecords.Count * 3);
            for (var move = 0; move < maxMoves; move++)
            {
                var overTail = -1;
                var overAmount = 0d;
                for (var tail = 0; tail < scale; tail++)
                {
                    var excess = usage[tail] - maximumUsage[tail];
                    if ((!validTailLookup[tail] && usage[tail] > 0) || excess > overAmount)
                    {
                        overTail = tail;
                        overAmount = !validTailLookup[tail] ? double.MaxValue : excess;
                    }
                }
                ...
                var recordIndex = -1;
                var recordStart = random.Next(groupRecords.Count);
                for (var offset = 0; offset < groupRecords.Count; offset++)
                {
                    var candidateIndex = (recordStart + offset) % groupRecords.Count;
                    if (tails[candidateIndex] == overTail)
                    {
                ...
                var bestTargetTail = -1;
                var bestPartnerTail = -1;
                var bestRecordAmount = 0d;
                var bestPartnerAmount = 0d;
                var bestScore = 0d;
                var targetStart = random.Next(validTails.Length);
                for (var targetOffset = 0; targetOffset < validTails.Length; targetOffset++)
                {
                    var targetTail = validTails[(targetStart + targetOffset) % validTails.Length];
                    if (usage[targetTail] >= maximumUsage[targetTail])
                    {
                        continue;
                    }

                    var positiveSteps = (targetTail - overTail + scale) % scale;
                    if (positiveSteps == 0)
                    {
                        continue;
                    }

                    var partnerStart = random.Next(groupRecords.Count);
                    for (var partnerOffset = 0; partnerOffset < groupRecords.Count; partnerOffset++)
                    {
                        var partnerIndex = (partnerStart + partnerOffset) % groupRecords.Count;
                        if (partnerIndex == recordIndex)
                        {
                            continue;
                        }
                        ...
                        {
                            var deltaSteps = option == 0 ? positiveSteps : positiveSteps - scale;
                            if (deltaSteps == 0)
                            {
                                continue;
                            }

                            var delta = deltaSteps / (double)scale;
                            var nextRecordAmount = RoundMoney(amounts[recordIndex] + delta);
                            var nextPartnerAmount = RoundMoney(amounts[partnerIndex] - delta);
                            if (!IsAmountWithinPrecisionBounds(nextRecordAmount, bounds[recordIndex])
                                || !IsAmountWithinPrecisionBounds(nextPartnerAmount, bounds[partnerIndex])
                                || requireBalanceSafe && !IsConfiguredPrecisionPairBalanceSafe(
                                    groupRecords[recordIndex],
                                    groupRecords[partnerIndex],
                                    group.Key.Sign,
                                    delta))
                            {
                                continue;
                            ...
                            var nextRecordTail = GetConfiguredPrecisionTail(nextRecordAmount, precision);
                            var nextPartnerTail = GetConfiguredPrecisionTail(nextPartnerAmount, precision);
                            if (nextRecordTail != targetTail
                                || nextPartnerTail < 0
                                || nextPartnerTail >= scale
                                || !validTailLookup[nextPartnerTail])
                            {
                                continue;
                            }

                            var score = CalculatePrecisionTailArrayMoveScore(
                                usage,
                                targetUsage,
                                overTail,
                                tails[partnerIndex],
                                nextRecordTail,
                                nextPartnerTail);
                            var recordTime = groupRecords[recordIndex].AccountTime;
                            var partnerTime = groupRecords[partnerIndex].AccountTime;
                            if (recordTime.HasValue && partnerTime.HasValue
                                && (recordTime.Value.Year != partnerTime.Value.Year
                                    || recordTime.Value.Month != partnerTime.Value.Month))
                            {
                                score += 0.05d;
                            }

                            if (bestPartnerIndex < 0 || score < bestScore - 0.0000001d)
                            {
                                bestPartnerIndex = partnerIndex;
                                bestTargetTail = nextRecordTail;
                                bestPartnerTail = nextPartnerTail;
                                bestRecordAmount = nextRecordAmount;
                                bestPartnerAmount = nextPartnerAmount;
                                bestScore = score;
                            }
                        }
                    }
                ...
                if (bestPartnerIndex < 0 || bestScore >= -0.0000001d)
                {
                    break;
                }

                usage[tails[recordIndex]]--;
                usage[tails[bestPartnerIndex]]--;
                usage[bestTargetTail]++;
                usage[bestPartnerTail]++;
                tails[recordIndex] = bestTargetTail;
                tails[bestPartnerIndex] = bestPartnerTail;
                amounts[recordIndex] = bestRecordAmount;
                amounts[bestPartnerIndex] = bestPartnerAmount;
                ApplySignedAmount(groupRecords[recordIndex], group.Key.Sign, bestRecordAmount);
                ApplySignedAmount(groupRecords[bestPartnerIndex], group.Key.Sign, bestPartnerAmount);
                changed = true;
            }
        }
    ...
    private static double CalculatePrecisionTailArrayMoveScore(
        IReadOnlyList<int> usage,
        IReadOnlyList<double> targetUsage,
        int oldFirstTail,
        int oldSecondTail,
        int newFirstTail,
        int newSecondTail)
    {
        Span<int> affected = stackalloc int[4];
        affected[0] = oldFirstTail;
        affected[1] = oldSecondTail;
        affected[2] = newFirstTail;
        affected[3] = newSecondTail;
        var before = 0d;
        var after = 0d;
        for (var index = 0; index < affected.Length; index++)
        {
            var tail = affected[index];
            var duplicate = false;
            for (var previous = 0; previous < index; previous++)
            {
                if (affected[previous] == tail)
                {
                    duplicate = true;
                    break;
                }
            ...
            var current = usage[tail];
            var next = current;
            if (tail == oldFirstTail)
            {
                next--;
            }

            if (tail == oldSecondTail)
            {
                next--;
            }

            if (tail == newFirstTail)
            {
                next++;
            }

            if (tail == newSecondTail)
            {
                next++;
            }

            before += Math.Pow(current - targetUsage[tail], 2d);
            after += Math.Pow(next - targetUsage[tail], 2d);
        }

        return after - before;
    }

    private static bool TryFindConfiguredPrecisionPairMove(
        FlowRecord record,
        FlowRecord partner,
        int sign,
        int precision,
        int targetTail,
        IReadOnlyDictionary<int, int> tailUsage,
        IReadOnlyDictionary<string, int> exactUsage,
        IReadOnlyDictionary<int, double> targetTailUsage,
        bool requireBalanceSafe,
        out double nextRecordAmount,
        out double nextPartnerAmount,
        out double score)
    {
        nextRecordAmount = 0;
        nextPartnerAmount = 0;
        score = 0;
        var scale = precision == 1 ? 10 : 100;
        var recordAmount = RoundMoney(Math.Abs(record.TradeMoney ?? 0));
        var partnerAmount = RoundMoney(Math.Abs(partner.TradeMoney ?? 0));
        var currentTail = GetConfiguredPrecisionTail(recordAmount, precision);
        var positiveSteps = (targetTail - currentTail + scale) % scale;
        if (positiveSteps == 0)
        {
            return false;
        }

        var recordBounds = GetRecordAmountBounds(record);
        var partnerBounds = GetRecordAmountBounds(partner);
        var bestScore = double.MaxValue;
        foreach (var deltaSteps in new[] { positiveSteps, positiveSteps - scale })
        {
            if (deltaSteps == 0)
            {
                continue;
            ...
            var delta = deltaSteps / (double)scale;
            var candidateRecordAmount = RoundMoney(recordAmount + delta);
            var candidatePartnerAmount = RoundMoney(partnerAmount - delta);
            if (!IsAmountWithinPrecisionBounds(candidateRecordAmount, recordBounds)
                || !IsAmountWithinPrecisionBounds(candidatePartnerAmount, partnerBounds)
                || requireBalanceSafe && !IsConfiguredPrecisionPairBalanceSafe(record, partner, sign, delta))
            {
                continue;
            }

            var partnerTail = GetConfiguredPrecisionTail(partnerAmount, precision);
            var nextRecordTail = GetConfiguredPrecisionTail(candidateRecordAmount, precision);
            var nextPartnerTail = GetConfiguredPrecisionTail(candidatePartnerAmount, precision);
            if (!tailUsage.ContainsKey(nextRecordTail) || !tailUsage.ContainsKey(nextPartnerTail))
            {
                continue;
            }

            var tailScore = CalculatePrecisionTailMoveScore(
                tailUsage,
                targetTailUsage,
                currentTail,
                partnerTail,
                nextRecordTail,
                nextPartnerTail);
            var exactScore = CalculatePrecisionExactMoveScore(
                exactUsage,
                record,
                partner,
                recordAmount,
                partnerAmount,
                candidateRecordAmount,
                candidatePartnerAmount);
            var candidateScore = tailScore + (exactScore * 0.15d);
            if (candidateScore < bestScore - 0.0000001d)
            {
                bestScore = candidateScore;
                nextRecordAmount = candidateRecordAmount;
                nextPartnerAmount = candidatePartnerAmount;
            }
        }
    ...
    private static bool IsAmountWithinPrecisionBounds(double amount, AmountBounds bounds)
    {
        return amount >= bounds.Min - 0.009d
            && amount <= bounds.Max + 0.009d
            && HasRequiredNonZeroPrecisionDigit(amount, bounds);
    }

    private static bool IsConfiguredPrecisionPairBalanceSafe(
        FlowRecord record,
        FlowRecord partner,
        int sign,
        double recordDelta)
    {
        var recordIsEarlier = (record.AccountTime ?? DateTime.MinValue) <= (partner.AccountTime ?? DateTime.MinValue);
        var earlierDelta = recordIsEarlier ? recordDelta : -recordDelta;
        return sign > 0 ? earlierDelta >= -0.0000001d : earlierDelta <= 0.0000001d;
    }

    private static double CalculatePrecisionTailMoveScore(
        IReadOnlyDictionary<int, int> usage,
        IReadOnlyDictionary<int, double> targetUsage,
        int oldFirstTail,
        int oldSecondTail,
        int newFirstTail,
        int newSecondTail)
    {
        var affectedTails = new[] { oldFirstTail, oldSecondTail, newFirstTail, newSecondTail }
            .Where(usage.ContainsKey)
            .Distinct()
            .ToArray();
        var before = affectedTails.Sum(tail => Math.Pow(usage[tail] - targetUsage.GetValueOrDefault(tail), 2d));
        var deltas = new Dictionary<int, int>();
        if (usage.ContainsKey(oldFirstTail))
        {
            deltas[oldFirstTail] = deltas.GetValueOrDefault(oldFirstTail) - 1;
        }

        if (usage.ContainsKey(oldSecondTail))
        {
            deltas[oldSecondTail] = deltas.GetValueOrDefault(oldSecondTail) - 1;
        }

        deltas[newFirstTail] = deltas.GetValueOrDefault(newFirstTail) + 1;
        deltas[newSecondTail] = deltas.GetValueOrDefault(newSecondTail) + 1;
        var after = affectedTails.Sum(tail => Math.Pow(
            usage[tail] + deltas.GetValueOrDefault(tail) - targetUsage.GetValueOrDefault(tail),
            2d));
        var invalidBefore = (usage.ContainsKey(oldFirstTail) ? 0 : 1) + (usage.ContainsKey(oldSecondTail) ? 0 : 1);
        return after - before - (invalidBefore * 1000d);
    }

    private static IReadOnlyDictionary<int, double> CreateConfiguredPrecisionTailTargets(
        int recordCount,
        int precision,
        IReadOnlyList<int> validTails,
        bool allowProtectedTails)
    {
        ...
        if (precision != 2)
        {
            var uniformTarget = recordCount / (double)validTails.Count;
            foreach (var tail in validTails)
            {
                result[tail] = uniformTarget;
            }

            return result;
        }

        var protectedTails = validTails.Where(IsProtectedNinetiesPrecisionTail).ToHashSet();
        var regularTails = validTails.Where(tail => !protectedTails.Contains(tail)).ToArray();
        if (allowProtectedTails && recordCount <= validTails.Count)
        {
            var uniformTarget = recordCount / (double)validTails.Count;
            foreach (var tail in validTails)
            {
        ...
        var protectedTotal = allowProtectedTails
            ? Math.Min(recordCount, protectedTails.Count)
            : 0;
        var regularTarget = regularTails.Length == 0
            ? 0d
            : Math.Max(0, recordCount - protectedTotal) / (double)regularTails.Length;
        foreach (var tail in validTails)
        {
            result[tail] = protectedTails.Contains(tail)
                ? allowProtectedTails ? 1d : 0d
                : regularTarget;
        }
    ...
    private static int GetConfiguredPrecisionTailMaximumUsage(double targetUsage)
    {
        return targetUsage <= 0.0000001d
            ? 0
            : Math.Max(1, (int)Math.Ceiling(targetUsage - 0.0000001d));
    }

    private static bool IsProtectedNinetiesPrecisionTail(int tail)
    {
        return tail is >= 91 and <= 99;
    }

    private static double CalculatePrecisionExactMoveScore(
        IReadOnlyDictionary<string, int> usage,
        FlowRecord first,
        FlowRecord second,
        double oldFirstAmount,
        double oldSecondAmount,
        double newFirstAmount,
        double newSecondAmount)
    {
        var oldFirstKey = CreatePrecisionExactUsageKey(first, oldFirstAmount);
        var oldSecondKey = CreatePrecisionExactUsageKey(second, oldSecondAmount);
        var newFirstKey = CreatePrecisionExactUsageKey(first, newFirstAmount);
        var newSecondKey = CreatePrecisionExactUsageKey(second, newSecondAmount);
        var keys = new[] { oldFirstKey, oldSecondKey, newFirstKey, newSecondKey }
            .Distinct(StringComparer.Ordinal)
            .ToArray();
        var before = keys.Sum(key => CalculateDuplicatePairCount(usage.GetValueOrDefault(key)));
        var deltas = new Dictionary<string, int>(StringComparer.Ordinal)
        {
            [oldFirstKey] = -1
        };
        deltas[oldSecondKey] = deltas.GetValueOrDefault(oldSecondKey) - 1;
        deltas[newFirstKey] = deltas.GetValueOrDefault(newFirstKey) + 1;
        deltas[newSecondKey] = deltas.GetValueOrDefault(newSecondKey) + 1;
        var after = keys.Sum(key => CalculateDuplicatePairCount(usage.GetValueOrDefault(key) + deltas.GetValueOrDefault(key)));
        return after - before;
    }

    private static double CalculateDuplicatePairCount(int count)
    {
        return count <= 1 ? 0d : count * (count - 1d) / 2d;
    }

    private static void UpdatePrecisionUsageBeforeMove(
        FlowRecord first,
        FlowRecord second,
        int precision,
        IDictionary<int, int> tailUsage,
        IDictionary<string, int> exactUsage,
        int delta)
    {
        foreach (var record in new[] { first, second })
        {
            var amount = RoundMoney(Math.Abs(record.TradeMoney ?? 0));
            var tail = GetConfiguredPrecisionTail(amount, precision);
            if (tailUsage.ContainsKey(tail))
            {
                tailUsage[tail] += delta;
            }

            var exactKey = CreatePrecisionExactUsageKey(record, amount);
            exactUsage.TryGetValue(exactKey, out var currentUsage);
            exactUsage[exactKey] = currentUsage + delta;
            if (exactUsage[exactKey] <= 0)
            {
                exactUsage.Remove(exactKey);
            }
        }
    }

    private static string CreatePrecisionExactUsageKey(FlowRecord record, double amount)
    {
        return string.Create(
            CultureInfo.InvariantCulture,
            $"{GetGeneratedAmountDistributionKey(record)}:{RoundMoney(amount):0.00}");
    }

    private static bool SpreadConfiguredPrecisionAmountsAcrossTimeline(
        IReadOnlyList<FlowRecord> records,
        Random random)
    {
        var changed = false;
        var exactTimes = new Dictionary<string, List<DateTime>>(StringComparer.Ordinal);
        var tailTimes = new Dictionary<string, List<DateTime>>(StringComparer.Ordinal);
        var timelineStart = records
            .Where(item => item.AccountTime.HasValue)
            .Select(item => item.AccountTime!.Value)
            .DefaultIfEmpty(DateTime.Today)
            .Min();
        var timelineEnd = records
            .Where(item => item.AccountTime.HasValue)
            .Select(item => item.AccountTime!.Value)
            .DefaultIfEmpty(timelineStart)
            .Max();
        var timelineSpanDays = Math.Max(1d, (timelineEnd - timelineStart).TotalDays);
        var exactOccurrenceCounts = records
            .Where(item => item.AccountTime.HasValue)
            .GroupBy(item =>
            {
                var bounds = GetRecordAmountBounds(item);
                return CreatePrecisionTimelineExactKey(
                    GetAmountSign(item.TradeMoney ?? 0),
                    Math.Clamp(bounds.Precision ?? 0, 1, 2),
                    Math.Abs(item.TradeMoney ?? 0));
            })
            .ToDictionary(group => group.Key, group => group.Count(), StringComparer.Ordinal);
        var tailOccurrenceCounts = records
            .Where(item => item.AccountTime.HasValue)
            .GroupBy(item =>
            {
                var bounds = GetRecordAmountBounds(item);
                var precision = Math.Clamp(bounds.Precision ?? 0, 1, 2);
                var tail = GetConfiguredPrecisionTail(Math.Abs(item.TradeMoney ?? 0), precision);
                return CreatePrecisionTimelineTailKey(GetAmountSign(item.TradeMoney ?? 0), precision, tail);
            })
            .ToDictionary(group => group.Key, group => group.Count(), StringComparer.Ordinal);
        var groups = records
            .Where(item => item.AccountTime.HasValue)
        ...
        foreach (var group in groups)
        {
            ...
            var remainingAmounts = groupRecords
                .Select(item => RoundMoney(Math.Abs(item.TradeMoney ?? 0)))
                .ToList();
            foreach (var record in groupRecords)
            {
                var recordTime = record.AccountTime!.Value;
                var originalAmount = RoundMoney(Math.Abs(record.TradeMoney ?? 0));
                var bestIndex = -1;
                var bestScore = double.MaxValue;
                for (var index = 0; index < remainingAmounts.Count; index++)
                {
                    var amount = remainingAmounts[index];
                    if (!IsAmountWithinPrecisionBounds(amount, GetRecordAmountBounds(record)))
                    {
                        continue;
                    }

                    var exactKey = CreatePrecisionTimelineExactKey(group.Key.Sign, group.Key.Precision, amount);
                    var tail = GetConfiguredPrecisionTail(amount, group.Key.Precision);
                    var tailKey = CreatePrecisionTimelineTailKey(group.Key.Sign, group.Key.Precision, tail);
                    var exactPreferredGap = CalculatePreferredPrecisionRepeatGapDays(
                        timelineSpanDays,
                        exactOccurrenceCounts.GetValueOrDefault(exactKey));
                    var exactMinimumGap = CalculateMinimumPrecisionRepeatGapDays(
                        timelineSpanDays,
                        exactOccurrenceCounts.GetValueOrDefault(exactKey));
                    var tailPreferredGap = CalculatePreferredPrecisionRepeatGapDays(
                        timelineSpanDays,
                        tailOccurrenceCounts.GetValueOrDefault(tailKey));
                    var tailMinimumGap = CalculateMinimumPrecisionRepeatGapDays(
                        timelineSpanDays,
                        tailOccurrenceCounts.GetValueOrDefault(tailKey));
                    var candidateScore = CalculatePrecisionTimelineCollisionPenalty(
                            recordTime,
                            exactTimes.GetValueOrDefault(exactKey),
                            sameTimeWeight: 1_000_000d,
                            sameDayWeight: 120_000d)
                        + CalculatePrecisionTimelineCollisionPenalty(
                            recordTime,
                            tailTimes.GetValueOrDefault(tailKey),
                            sameTimeWeight: 500_000d,
                            sameDayWeight: 60_000d)
                        + CalculatePrecisionTimelineSpacingPenalty(
                            recordTime,
                            exactTimes.GetValueOrDefault(exactKey),
                            exactPreferredGap,
                            1600d)
                        ...
                        + CalculatePrecisionTimelineSpacingPenalty(
                            ...
                            tailMinimumGap,
                            1800d)
                        + (Math.Abs(amount - originalAmount) * 0.0001d)
                        + (random.NextDouble() * 0.001d);
                    if (candidateScore < bestScore - 0.0000001d)
                    {
                        bestIndex = index;
                        bestScore = candidateScore;
                ...
                var selectedAmount = remainingAmounts[bestIndex];
                remainingAmounts.RemoveAt(bestIndex);
                if (Math.Abs(selectedAmount - originalAmount) > 0.009d)
                {
                    ApplySignedAmount(record, group.Key.Sign, selectedAmount);
                    changed = true;
                }

                var selectedExactKey = CreatePrecisionTimelineExactKey(group.Key.Sign, group.Key.Precision, selectedAmount);
                var selectedTail = GetConfiguredPrecisionTail(selectedAmount, group.Key.Precision);
                var selectedTailKey = CreatePrecisionTimelineTailKey(group.Key.Sign, group.Key.Precision, selectedTail);
                AddPrecisionTimelineTime(exactTimes, selectedExactKey, recordTime);
                AddPrecisionTimelineTime(tailTimes, selectedTailKey, recordTime);
            }
        }
    ...
    private static string CreatePrecisionTimelineExactKey(int sign, int precision, double amount)
    {
        return string.Create(CultureInfo.InvariantCulture, $"{sign}:{precision}:{RoundMoney(amount):0.00}");
    }

    private static string CreatePrecisionTimelineTailKey(int sign, int precision, int tail)
    {
        return string.Create(CultureInfo.InvariantCulture, $"{sign}:{precision}:{tail}");
    }

    private static double CalculatePreferredPrecisionRepeatGapDays(double timelineSpanDays, int occurrenceCount)
    {
        if (occurrenceCount <= 1)
        {
            return PreferredPrecisionRepeatGapDays;
        }

        var availableGap = timelineSpanDays / Math.Max(1, occurrenceCount - 1);
        return Math.Min(PreferredPrecisionRepeatGapDays, Math.Max(0.25d, availableGap * 0.90d));
    }

    private static double CalculateMinimumPrecisionRepeatGapDays(double timelineSpanDays, int occurrenceCount)
    {
        if (occurrenceCount <= 1)
        {
            return MinimumPrecisionRepeatGapDays;
        }

        var availableGap = timelineSpanDays / Math.Max(1, occurrenceCount - 1);
        return Math.Min(MinimumPrecisionRepeatGapDays, Math.Max(0.25d, availableGap * 0.75d));
    }

    private static double CalculatePrecisionTimelineCollisionPenalty(
        DateTime time,
        IReadOnlyList<DateTime>? existingTimes,
        double sameTimeWeight,
        double sameDayWeight)
    {
        if (existingTimes is null || existingTimes.Count == 0)
        {
            return 0d;
        }
        ...
        {
            if (Math.Abs((time - existingTime).TotalSeconds) < 1d)
            {
                penalty += sameTimeWeight;
            }
            else if (time.Date == existingTime.Date)
            {
                penalty += sameDayWeight;
            }
        }
    ...
    private static double CalculatePrecisionTimelineSpacingPenalty(
        DateTime time,
        IReadOnlyList<DateTime>? existingTimes,
        double preferredGapDays,
        double weight)
    {
        if (existingTimes is null || existingTimes.Count == 0)
        {
        ...
        var nearestGap = existingTimes.Min(item => Math.Abs((time - item).TotalDays));
        if (nearestGap >= preferredGapDays)
        {
            return 0d;
        }

        return (preferredGapDays - nearestGap + 1d) * weight;
    }

    private static void AddPrecisionTimelineTime(
        IDictionary<string, List<DateTime>> values,
        string key,
        DateTime time)
    {
        if (!values.TryGetValue(key, out var times))
        {
            times = [];
            values[key] = times;
        }
    ...
    private static bool ImproveConfiguredPrecisionTailSpacing(
        IReadOnlyList<FlowRecord> records,
        Random random,
        bool requireBalanceSafe)
    {
        // The first pass is allowed to permute configured tails. If that result makes an
        // already-tight balance negative, the caller restores the snapshot and invokes the
        // balance-safe pass. Keeping the restored values is safer than running the old
        // quadratic pair search again.
        if (requireBalanceSafe)
        {
            return false;
        }
        ...
        foreach (var group in groups)
        {
            ...
            var validTails = Enumerable.Range(1, scale - 1)
                .Where(tail => tail % 10 != 0)
                .ToHashSet();
            var states = groupRecords
                .Select(record =>
                {
                    var amount = RoundMoney(Math.Abs(record.TradeMoney ?? 0));
                    var amountUnits = Convert.ToInt64(Math.Round(amount * scale, MidpointRounding.AwayFromZero));
                    return new
                    {
                        Record = record,
                        Bounds = GetRecordAmountBounds(record),
                        ...
                        WholeUnits = amountUnits - Math.Abs(amountUnits % scale),
                        Tail = GetConfiguredPrecisionTail(amount, precision),
                        SourceKey = GetGeneratedAmountDistributionKey(record),
                        Time = record.AccountTime!.Value
                    };
                })
                .ToList();
            if (states.Any(state => !validTails.Contains(state.Tail)))
            {
                continue;
            }

            var remainingTailCounts = states
                .GroupBy(state => state.Tail)
                .ToDictionary(items => items.Key, items => items.Count());
            var totalTailCounts = remainingTailCounts.ToDictionary(item => item.Key, item => item.Value);
            var assignedTailCounts = totalTailCounts.Keys.ToDictionary(tail => tail, _ => 0);
            var lastTailTime = new Dictionary<int, DateTime>();
            var exactUsage = new Dictionary<(string SourceKey, long Cents), int>();
            var lastExactTime = new Dictionary<(string SourceKey, long Cents), DateTime>();
            var timelineStart = states[0].Time;
            var timelineEnd = states[^1].Time;
            var timelineSpanDays = Math.Max(
                1d,
                (timelineEnd - timelineStart).TotalDays);
            var assignments = new List<(FlowRecord Record, double Amount)>(states.Count);
            var complete = true;
            foreach (var state in states)
            {
                ...
                var bestAmount = 0d;
                var bestScore = double.MaxValue;
                foreach (var pair in remainingTailCounts)
                {
                    var tail = pair.Key;
                    if (pair.Value <= 0)
                    {
                    ...
                    var candidateUnits = state.WholeUnits + tail;
                    var candidateAmount = RoundMoney(candidateUnits / (double)scale);
                    if (!IsAmountWithinPrecisionBounds(candidateAmount, state.Bounds))
                    {
                        continue;
                    }

                    var totalOccurrences = totalTailCounts[tail];
                    var occurrenceIndex = assignedTailCounts[tail] + 1;
                    var idealTime = timelineStart.AddDays(
                        timelineSpanDays * occurrenceIndex / Math.Max(1d, totalOccurrences + 1d));
                    var preferredGap = CalculatePreferredPrecisionRepeatGapDays(
                        timelineSpanDays,
                        totalOccurrences);
                    var minimumGap = CalculateMinimumPrecisionRepeatGapDays(
                        timelineSpanDays,
                        totalOccurrences);
                    var score = Math.Abs((state.Time - idealTime).TotalDays) * 4d;
                    if (lastTailTime.TryGetValue(tail, out var previousTailTime))
                    {
                        var gap = (state.Time - previousTailTime).TotalDays;
                        if (state.Time.Date == previousTailTime.Date)
                        {
                            score += 500_000d;
                        }
                        ...
                        if (gap < minimumGap)
                        {
                            score += (minimumGap - gap + 1d) * 12_000d;
                        }
                        else if (gap < preferredGap)
                        {
                            score += (preferredGap - gap + 1d) * 1800d;
                        }
                    }

                    var cents = Convert.ToInt64(Math.Round(candidateAmount * 100d, MidpointRounding.AwayFromZero));
                    var exactKey = (state.SourceKey, cents);
                    var existingExactUsage = exactUsage.GetValueOrDefault(exactKey);
                    score += CalculateDuplicatePairCount(existingExactUsage + 1) * 6000d;
                    if (lastExactTime.TryGetValue(exactKey, out var previousExactTime))
                    {
                        var exactGap = (state.Time - previousExactTime).TotalDays;
                        if (state.Time.Date == previousExactTime.Date)
                        {
                            score += 1_000_000d;
                        }

                        if (exactGap < MinimumPrecisionRepeatGapDays)
                        {
                            score += (MinimumPrecisionRepeatGapDays - exactGap + 1d) * 20_000d;
                        }
                    }

                    score += Math.Abs(candidateAmount - state.Amount) * 0.001d;
                    score -= pair.Value / (double)totalOccurrences;
                    score += random.NextDouble() * 0.001d;
                ...
                assignments.Add((state.Record, bestAmount));
                remainingTailCounts[bestTail]--;
                assignedTailCounts[bestTail]++;
                lastTailTime[bestTail] = state.Time;
                var bestCents = Convert.ToInt64(Math.Round(bestAmount * 100d, MidpointRounding.AwayFromZero));
                var bestExactKey = (state.SourceKey, bestCents);
                exactUsage[bestExactKey] = exactUsage.GetValueOrDefault(bestExactKey) + 1;
                lastExactTime[bestExactKey] = state.Time;
            }

            if (!complete || remainingTailCounts.Values.Any(count => count != 0))
            {
                continue;
            }

            var originalTotal = RoundMoney(states.Sum(state => state.Amount));
            var assignedTotal = RoundMoney(assignments.Sum(item => item.Amount));
            if (Math.Abs(originalTotal - assignedTotal) > 0.009d)
            {
                continue;
            }
            ...
            {
                var originalAmount = RoundMoney(Math.Abs(assignment.Record.TradeMoney ?? 0));
                if (Math.Abs(originalAmount - assignment.Amount) <= 0.009d)
                {
                    continue;
                }

                ApplySignedAmount(assignment.Record, group.Key.Sign, assignment.Amount);
                changed = true;
            }
        }
    ...
    private static bool TryFindConfiguredPrecisionTailSwap(
        FlowRecord record,
        FlowRecord partner,
        int sign,
        int precision,
        int scale,
        IReadOnlyDictionary<int, List<DateTime>> tailTimes,
        IReadOnlyDictionary<string, int> exactUsage,
        double timelineSpanDays,
        bool requireBalanceSafe,
        out double nextRecordAmount,
        out double nextPartnerAmount,
        out double score)
    {
        nextRecordAmount = 0d;
        nextPartnerAmount = 0d;
        score = 0d;
        var recordAmount = RoundMoney(Math.Abs(record.TradeMoney ?? 0));
        var partnerAmount = RoundMoney(Math.Abs(partner.TradeMoney ?? 0));
        var recordTail = GetConfiguredPrecisionTail(recordAmount, precision);
        var partnerTail = GetConfiguredPrecisionTail(partnerAmount, precision);
        if (recordTail == partnerTail)
        {
            return false;
        }
        ...
        var recordBounds = GetRecordAmountBounds(record);
        var partnerBounds = GetRecordAmountBounds(partner);
        var currentTemporalScore = CalculatePrecisionTailTemporalScore(
                tailTimes.GetValueOrDefault(recordTail) ?? [],
                timelineSpanDays)
            + CalculatePrecisionTailTemporalScore(
                tailTimes.GetValueOrDefault(partnerTail) ?? [],
                timelineSpanDays);
        var positiveSteps = (partnerTail - recordTail + scale) % scale;
        var bestScore = double.MaxValue;
        foreach (var deltaSteps in new[] { positiveSteps, positiveSteps - scale })
        {
            if (deltaSteps == 0)
            ...
            if (!IsAmountWithinPrecisionBounds(candidateRecordAmount, recordBounds)
                || !IsAmountWithinPrecisionBounds(candidatePartnerAmount, partnerBounds)
                || GetConfiguredPrecisionTail(candidateRecordAmount, precision) != partnerTail
                || GetConfiguredPrecisionTail(candidatePartnerAmount, precision) != recordTail
                || requireBalanceSafe && !IsConfiguredPrecisionPairBalanceSafe(record, partner, sign, delta))
            {
                continue;
            }

            var nextRecordTailTimes = CreateMovedPrecisionTailTimes(
                tailTimes.GetValueOrDefault(recordTail) ?? [],
                record.AccountTime!.Value,
                partner.AccountTime!.Value);
            var nextPartnerTailTimes = CreateMovedPrecisionTailTimes(
                tailTimes.GetValueOrDefault(partnerTail) ?? [],
                partner.AccountTime!.Value,
                record.AccountTime!.Value);
            var nextTemporalScore = CalculatePrecisionTailTemporalScore(nextRecordTailTimes, timelineSpanDays)
                + CalculatePrecisionTailTemporalScore(nextPartnerTailTimes, timelineSpanDays);
            var exactScore = CalculatePrecisionExactMoveScore(
                exactUsage,
                record,
                partner,
                ...
                candidateRecordAmount,
                candidatePartnerAmount);
            var candidateScore = nextTemporalScore - currentTemporalScore + (exactScore * 250d);
            if (candidateScore < bestScore - 0.0000001d)
            {
                bestScore = candidateScore;
                nextRecordAmount = candidateRecordAmount;
    ...
    private static double CalculateNearestOtherPrecisionTimeGap(
        DateTime time,
        IReadOnlyList<DateTime> times)
    {
        ...
        {
            if (!skippedSelf && candidate == time)
            {
                skippedSelf = true;
                continue;
            }

            nearestGap = Math.Min(nearestGap, Math.Abs((time - candidate).TotalDays));
        }

        return nearestGap;
    }

    private static double CalculatePrecisionTailTemporalScore(
        IReadOnlyList<DateTime> times,
        double timelineSpanDays)
    {
        if (times.Count < 2)
        ...
        var ordered = times.OrderBy(item => item).ToArray();
        var preferredGap = CalculatePreferredPrecisionRepeatGapDays(timelineSpanDays, ordered.Length);
        var minimumGap = CalculateMinimumPrecisionRepeatGapDays(timelineSpanDays, ordered.Length);
        var score = 0d;
        for (var index = 1; index < ordered.Length; index++)
        {
            var previous = ordered[index - 1];
            var current = ordered[index];
            score += CalculatePrecisionTimelineCollisionPenalty(
                current,
                [previous],
                sameTimeWeight: 1_000_000d,
                sameDayWeight: 120_000d);
            score += CalculatePrecisionTimelineSpacingPenalty(current, [previous], preferredGap, 1600d);
            score += CalculatePrecisionTimelineSpacingPenalty(
                current,
                [previous],
                minimumGap,
                8000d);
    ...
    private static List<DateTime> CreateMovedPrecisionTailTimes(
        IReadOnlyList<DateTime> times,
        DateTime remove,
        DateTime add)
    {
        var result = times.ToList();
    ...
    private static void MovePrecisionTailTime(
        IDictionary<int, List<DateTime>> tailTimes,
        int oldTail,
        int newTail,
        DateTime time)
    {
        tailTimes[oldTail].Remove(time);
        if (!tailTimes.TryGetValue(newTail, out var targetTimes))
        {
            targetTimes = [];
            tailTimes[newTail] = targetTimes;
        }
    ...
    private static void UpdatePrecisionExactUsage(
        FlowRecord first,
        FlowRecord second,
        IDictionary<string, int> exactUsage,
        int delta)
    {
        ...
        {
            var amount = RoundMoney(Math.Abs(record.TradeMoney ?? 0));
            var key = CreatePrecisionExactUsageKey(record, amount);
            exactUsage.TryGetValue(key, out var currentUsage);
            exactUsage[key] = currentUsage + delta;
            if (exactUsage[key] <= 0)
            {
                exactUsage.Remove(key);
            }
        }
    }
    ...
    private static void ExpandWechatReferenceVolume(
        FlowAutoGenerationRequest request,
        DateTime start,
        DateTime end,
        Random random,
        ...
        AmountUsageTracker amountUsage)
    {
        ...
        var dayCount = Math.Max(1, (end.Date - start.Date).Days + 1);
        var targetTotal = Math.Max(
            records.Count,
            (int)Math.Round(
                dayCount * (21.2d + (random.NextDouble() * 1.2d)),
                MidpointRounding.AwayFromZero));
        var incomeRatio = 0.54d + (random.NextDouble() * 0.02d);
        var targetIncomeCount = (int)Math.Round(targetTotal * incomeRatio, MidpointRounding.AwayFromZero);
        var targetExpenseCount = Math.Max(0, targetTotal - targetIncomeCount);

        ExpandWechatDirectionVolume(
            request,
            start,
            end,
            targetIncomeCount,
        ...
        ConsolidateWechatHighExpenseRecords(records, random);
        PruneZeroAmountRecords(records);
    }

    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    private static void ExpandWechatDirectionVolume(
        FlowAutoGenerationRequest request,
        DateTime start,
        DateTime end,
        int targetCount,
        ...
        AmountUsageTracker amountUsage)
    {
        var currentCount = records.Count(item =>
            !IsSystemInterestRecord(item)
            && GetAmountSign(item.TradeMoney ?? 0) == (isIncome ? 1 : -1));
        var missingCount = Math.Max(0, targetCount - currentCount);
        if (missingCount <= 0)
        {
            return;
        }

        var lowRules = request.References
            .Where(item => item.IsCheck && IsIncomeRuleSafe(item) == isIncome)
            .Where(item => GetRequiredMonthlyCount(item) <= 0)
            .Select(item => TryCreateCapacityTierAmountSlice(item, ReferenceCapacityTier.Low, out var slice)
                ? slice
                : null)
            .Where(item => item is not null)
            .Cast<GenerateReferenceRule>()
            .OrderBy(item => GetRuleAmountBounds(item).Max)
            .ThenBy(_ => random.Next())
            .ToList();
        if (lowRules.Count == 0)
        {
        ...
        var donors = records
            ...
            .Where(item => Math.Abs(item.TradeMoney ?? 0) > GetRecordAmountBounds(item).Min + 0.009d)
            .ToList();
        var donorCapacity = RoundMoney(donors.Sum(item =>
            Math.Max(0, Math.Abs(item.TradeMoney ?? 0) - GetRecordAmountBounds(item).Min)));
        if (donorCapacity <= 0.009d)
        {
            return;
        }

        var added = new List<FlowRecord>(missingCount);
        var addedTotal = 0d;
        var failedRules = 0;
        for (var index = 0; index < missingCount && failedRules < lowRules.Count; index++)
        {
            var rule = lowRules[index % lowRules.Count];
            var remainingCapacity = RoundMoney(donorCapacity - addedTotal);
            if (remainingCapacity < GetRuleAmountBounds(rule).Min - 0.009d)
            {
                failedRules++;
                index--;
                continue;
            ...
            scheduleState.Register(record);
            amountUsage.Register(record);
            addedTotal = RoundMoney(addedTotal + amount);
        }

        if (added.Count == 0 || addedTotal <= 0.009d)
        {
            return;
        }

        var donorTotalBefore = RoundMoney(donors.Sum(item => Math.Abs(item.TradeMoney ?? 0)));
        DecreaseSignedRecordsWithinBounds(donors, isIncome, addedTotal, allowDropOptional: false);
        var donorTotalAfter = RoundMoney(donors.Sum(item => Math.Abs(item.TradeMoney ?? 0)));
        var transferred = RoundMoney(Math.Max(0, donorTotalBefore - donorTotalAfter));
        if (transferred <= 0.009d)
        {
            foreach (var record in added)
            {
                ApplySignedAmount(record, isIncome ? 1d : -1d, 0);
            }

            return;
        }

        RebalanceSignedRecords(added, transferred);
    }

    private static void ConsolidateWechatHighExpenseRecords(List<FlowRecord> records, Random random)
    {
        var candidates = records
            .Where(IsGeneratedReferenceRecord)
            .Where(item => !IsRequiredGeneratedRecord(item))
            .Where(item => !IsSystemInterestRecord(item))
            .Where(item => item.TradeMoney < -0.009d)
            .Where(item => GetGeneratedAmountCapacityTier(Math.Abs(item.TradeMoney ?? 0)) == ReferenceCapacityTier.High)
            .ToList();
        ...
        var total = RoundMoney(candidates.Sum(item => Math.Abs(item.TradeMoney ?? 0)));
        var sourceAverages = candidates
            .GroupBy(GetGeneratedAmountDistributionKey)
            .Select(group => EstimateBalancedRangeAverage(GetRecordAmountBounds(group.First())))
            .Where(item => item > 0.009d)
            .ToList();
        var targetAverage = sourceAverages.Count == 0
            ? candidates.Average(item => Math.Abs(item.TradeMoney ?? 0))
            : sourceAverages.Average();
        targetAverage *= 0.96d + (random.NextDouble() * 0.08d);
        var targetCount = Math.Clamp(
            (int)Math.Ceiling(total / Math.Max(1d, targetAverage)),
            sourceAverages.Count,
            candidates.Count);
        ...
        while (candidates.Count > targetCount)
        {
            var removable = candidates
                .Where(item => sourceCounts.GetValueOrDefault(GetGeneratedAmountDistributionKey(item)) > 1)
                .OrderByDescending(item => sourceCounts.GetValueOrDefault(GetGeneratedAmountDistributionKey(item)))
                .ThenBy(item => Math.Abs(item.TradeMoney ?? 0))
                .ThenBy(_ => random.Next())
                .FirstOrDefault();
            ...
            var maximumAfterRemoval = RoundMoney(candidates
                .Where(item => !ReferenceEquals(item, removable))
                .Sum(item => GetRecordAmountBounds(item).Max));
            if (maximumAfterRemoval < total - 0.009d)
            {
                break;
            }

            var sourceKey = GetGeneratedAmountDistributionKey(removable);
            sourceCounts[sourceKey]--;
            candidates.Remove(removable);
            ApplySignedAmount(removable, -1d, 0);
        }

        RebalanceSignedRecords(candidates, total);
    }

    private static void CompactWechatReferenceVolume(
        FlowAutoGenerationRequest request,
        DateTime start,
        DateTime end,
        Random random,
        List<FlowRecord> records)
    {
        ...
        var dayCount = Math.Max(1, (end.Date - start.Date).Days + 1);
        var targetMaximum = (int)Math.Ceiling(dayCount * 23.5d);
        var excessCount = Math.Max(0, records.Count - targetMaximum);
        if (excessCount <= 0)
        {
            return;
        }

        var incomeCount = records.Count(item => (item.TradeMoney ?? 0) > 0.009d);
        var expenseCount = records.Count(item => (item.TradeMoney ?? 0) < -0.009d);
        var targetIncomeCount = (int)Math.Round(targetMaximum * 0.55d, MidpointRounding.AwayFromZero);
        var directions = new[]
            {
                (IsIncome: true, Surplus: incomeCount - targetIncomeCount),
                (IsIncome: false, Surplus: expenseCount - (targetMaximum - targetIncomeCount))
            }
            .OrderByDescending(item => item.Surplus)
            .ToList();
        ...
        foreach (var direction in directions)
        {
            ...
            var requested = direction.Surplus > 0
                ? Math.Min(excessCount, direction.Surplus)
                : excessCount;
            var removed = CompactWechatDirectionVolume(
                request,
                direction.IsIncome,
                requested,
                random,
    ...
    private static int CompactWechatDirectionVolume(
        FlowAutoGenerationRequest request,
        bool isIncome,
        int requestedCount,
        Random random,
        List<FlowRecord> records)
    {
        ...
        var rulesByIndex = request.References
        ...
        var candidates = records
            ...
            {
                Record = item,
                SourceIndex = item.ExtraFields.TryGetValue(SourceIndexField, out var sourceIndexText)
                    && ParseInt(sourceIndexText) is { } sourceIndex
                        ? sourceIndex
                        : -1
            })
            .Where(item => item.SourceIndex >= 0 && rulesByIndex.ContainsKey(item.SourceIndex))
            .OrderByDescending(item => sourceCounts.GetValueOrDefault(item.SourceIndex))
            .ThenByDescending(item => item.Record.AccountTime.HasValue
                ? dayCounts.GetValueOrDefault(item.Record.AccountTime.Value.Date)
                : 0)
            .ThenBy(item => Math.Abs(item.Record.TradeMoney ?? 0))
            .ThenBy(_ => random.Next())
            .ToList();
        if (candidates.Count == 0)
        {
        ...
        var selected = new List<FlowRecord>(Math.Min(requestedCount, candidates.Count));
        foreach (var candidate in candidates)
        {
        ...
        var recipients = records
            ...
            .Where(item => GetGeneratedAmountCapacityTier(Math.Abs(item.TradeMoney ?? 0)) != ReferenceCapacityTier.Low)
            .ToList();
        var recipientCapacity = RoundMoney(recipients.Sum(item =>
            Math.Max(0, GetRecordAmountBounds(item).Max - Math.Abs(item.TradeMoney ?? 0))));
        while (selected.Count > 0
               && selected.Sum(item => Math.Abs(item.TradeMoney ?? 0)) > recipientCapacity + 0.009d)
        {
            selected.RemoveAt(selected.Count - 1);
        }

        if (selected.Count == 0 || recipients.Count == 0)
        {
            return 0;
        }

        var originalRecords = records.ToList();
        var amountSnapshot = CaptureRecordAmountSnapshot(originalRecords);
        var transferAmount = RoundMoney(selected.Sum(item => Math.Abs(item.TradeMoney ?? 0)));
        var recipientTotal = RoundMoney(recipients.Sum(item => Math.Abs(item.TradeMoney ?? 0)));
        foreach (var record in selected)
        {
            records.Remove(record);
        }

        IncreaseSignedRecordsWithinBounds(recipients, isIncome, transferAmount);
        var transferred = RoundMoney(recipients.Sum(item => Math.Abs(item.TradeMoney ?? 0)) - recipientTotal);
        var remaining = RoundMoney(Math.Max(0, transferAmount - transferred));
        if (remaining > 0.009d)
        {
            var residualRecipients = records
                .Where(IsGeneratedReferenceRecord)
                .Where(item => !IsSystemInterestRecord(item))
                .Where(item => GetAmountSign(item.TradeMoney ?? 0) == sign)
                .Where(item => GetGeneratedAmountCapacityTier(Math.Abs(item.TradeMoney ?? 0)) == ReferenceCapacityTier.Low)
                .Where(item => GetRecordAmountBounds(item).Max > Math.Abs(item.TradeMoney ?? 0) + 0.009d)
                .OrderBy(item => GetRecordAmountUnit(item))
                .ThenBy(item => Math.Abs(item.TradeMoney ?? 0))
                .ToList();
            var residualTotal = RoundMoney(residualRecipients.Sum(item => Math.Abs(item.TradeMoney ?? 0)));
            IncreaseSignedRecordsWithinBounds(residualRecipients, isIncome, remaining);
            transferred = RoundMoney(transferred
                + residualRecipients.Sum(item => Math.Abs(item.TradeMoney ?? 0))
                - residualTotal);
        }

        if (Math.Abs(transferred - transferAmount) > 0.009d)
        {
            records.Clear();
    ...
    private static bool EnforceActualMonthlyIncomeCap(
        List<FlowRecord> records,
        FlowAutoGenerationRequest request,
        MonthlyAmountPlan monthlyPlan,
        double openingBalance,
        Random random)
    {
        if (records.Count == 0
            || monthlyPlan.Months.Count == 0
            || monthlyPlan.MonthlyIncomeCap <= 0.009d)
        {
            return false;
        }
        ...
        var originalAmounts = CaptureRecordAmountSnapshot(records);
        var originalTimes = records
                     ...
                     .OrderByDescending(index => monthlyTotals[index] - monthlyPlan.MonthlyIncomeCap))
        {
            ...
            var movableRecords = records
                ...
                .Where(item => IsRecordInRange(item, source.Start, source.End))
                .Select(item => new
                {
                    Record = item,
                    Rule = ResolveRecordSourceRule(request, item) as GenerateReferenceRule,
            ...
            foreach (var candidate in movableRecords)
            {
                ...
                var destinationIndexes = Enumerable.Range(0, monthlyPlan.Months.Count)
                    ...
                    .ThenBy(_ => random.Next())
                    .ToList();
                if (destinationIndexes.Count == 0 || !candidate.Record.AccountTime.HasValue)
                {
                    continue;
                }

                var destinationIndex = destinationIndexes[0];
                var destination = monthlyPlan.Months[destinationIndex];
                var originalTime = candidate.Record.AccountTime.Value;
                ...
                monthlyTotals[sourceIndex] = RoundMoney(monthlyTotals[sourceIndex] - candidate.Amount);
                monthlyTotals[destinationIndex] = RoundMoney(monthlyTotals[destinationIndex] + candidate.Amount);
            }
        }
                     ...
                     .OrderByDescending(index => monthlyTotals[index] - monthlyPlan.MonthlyIncomeCap))
        {
            ...
            var donors = records
                ...
                .Where(item => IsRecordInRange(item, source.Start, source.End))
                .OrderBy(item => IsRequiredGeneratedRecord(item) ? 1 : 0)
                .ThenByDescending(item => GetRecordReducibleSignedCapacity(item))
                .ThenBy(_ => random.Next())
                .ToList();
            var before = SumIncome(donors);
            DecreaseSignedRecordsWithinBounds(donors, isIncome: true, excess, allowDropOptional: true);
            var after = SumIncome(donors);
            var reduced = RoundMoney(before - after);
            monthlyTotals[sourceIndex] = RoundMoney(monthlyTotals[sourceIndex] - reduced);
            amountToRedistribute = RoundMoney(amountToRedistribute + reduced);
        }
                     ...
                     .ThenBy(_ => random.Next()))
        {
            ...
            var recipients = records
                .Where(item => item.TradeMoney > 0.009d)
                .Where(item => !IsSystemInterestRecord(item))
                .Where(item => IsRecordInRange(item, destination.Start, destination.End))
                .OrderByDescending(item => RoundMoney(GetRecordAmountBounds(item).Max - Math.Abs(item.TradeMoney ?? 0)))
                .ThenBy(_ => random.Next())
                .ToList();
            var before = SumIncome(recipients);
            IncreaseSignedRecordsWithinBounds(recipients, isIncome: true, requested);
            var after = SumIncome(recipients);
            var added = RoundMoney(after - before);
            monthlyTotals[destinationIndex] = RoundMoney(monthlyTotals[destinationIndex] + added);
        ...
        {
            RestoreRecordAmounts(originalAmounts);
            foreach (var original in originalTimes)
            {
                original.Record.AccountTime = original.AccountTime;
            }
            ApplyBalances(records, openingBalance);
            return false;
        }
        ...
        AmountUsageTracker amountUsage)
    {
        ...
        for (var pass = 0; pass < 6; pass++)
        {
            ...
            // Keep remainder absorption inside each month's own target. A global increase
            // here would push the remainder into late records and flatten the balance wave.
            IncreaseSignedRecordsWithinMonthlyTargets(
                records,
                monthlyPlan,
                targetTotal,
                isIncome,
            ...
            // Only use the global capacity as the final exact-total fallback after every
            // monthly generation pass has been exhausted.
            if (pass == 5)
            {
        ...
        AmountUsageTracker amountUsage)
    {
        ...
        {
            var totalBeforePass = GetNativeSignedTotal(records, isIncome);
            for (var monthIndex = 0; monthIndex < monthlyPlan.Months.Count; monthIndex++)
            {
            ...
            var totalAfterPass = GetNativeSignedTotal(records, isIncome);
            if (totalAfterPass <= totalBeforePass + 0.009d)
            {
                break;
            }
        }
    ...
    private static bool IncreaseSignedRecordsWithinMonthlyTargets(
        IEnumerable<FlowRecord> records,
        MonthlyAmountPlan monthlyPlan,
        double targetTotal,
        bool isIncome,
    ...
    {
        var remaining = RoundMoney(Math.Max(0, amount));
        if (remaining <= 0.009d || monthlyPlan.Months.Count == 0)
        {
            return remaining <= 0.009d;
        }
        ...
        var monthlyTargets = ScaleNativeTargets(
            ...
            targetTotal,
            monthlyPlan.Months.Count);
        var materialized = records as IReadOnlyCollection<FlowRecord> ?? records.ToList();

        for (var monthIndex = 0; monthIndex < monthlyPlan.Months.Count && remaining > 0.009d; monthIndex++)
        {
            var month = monthlyPlan.Months[monthIndex];
            var monthRecords = materialized
                .Where(item => IsRecordInRange(item, month.Start, month.End))
                .ToList();
            var current = GetNativeSignedTotal(monthRecords, isIncome);
            var deficit = RoundMoney(Math.Max(0, monthlyTargets[monthIndex] - current));
            var requested = RoundMoney(Math.Min(deficit, remaining));
            if (requested <= 0.009d)
            {
                continue;
            }

            var before = GetNativeSignedTotal(monthRecords, isIncome);
            IncreaseSignedRecordsWithinBounds(monthRecords, isIncome, requested);
            var after = GetNativeSignedTotal(monthRecords, isIncome);
        ...
        Random random)
    {
        ...
        var groups = records
            ...
            .Select(group =>
            {
                ...
                var expected = RoundMoney(EstimateBalancedRangeAverage(bounds) * groupRecords.Count);
                return new
                {
        ...
        Random random)
    {
        ...
        var estimatedCount = (int)Math.Ceiling(targetAmount / weightedAverage);
        var countVariation = isIncome
            ? 1.15d
            : 0.92d + (random.NextDouble() * 0.16d);
        var totalToAdd = Math.Max(
            groups.Count,
            (int)Math.Round(estimatedCount * countVariation, MidpointRounding.AwayFromZero));
        ...
        for (var attempt = 0; attempt < 128; attempt++)
        {
            ...
            if (!isIncome
                && dailySlots.Count > 0
                && groupsByTier.ContainsKey(ReferenceCapacityTier.Low))
            {
                // Daily small expenses are an independent coverage target. They must
                // not inflate the medium/high record counts through the tier ratio.
                counts[ReferenceCapacityTier.Low] = Math.Max(
                    counts[ReferenceCapacityTier.Low],
                    dailySlots.Count);
            }
        ...
        Random random)
    {
        ...
        foreach (var (tier, group) in groupsByTier)
        {
            ...
            var dailySlotLimit = tier == ReferenceCapacityTier.Low
                ? Math.Min(count, dailySlots.Count)
                : 0;
        ...
        int count)
    {
        ...
        for (var index = 0; index < count; index++)
        {
            ...
            minimum += bounds.Min;
            expected += EstimateBalancedRangeAverage(bounds);
            maximum += bounds.Max;
        }
    ...
    private static double EstimateRuleSequenceAverage(IReadOnlyList<GenerateReferenceRule> rules)
    {
        return rules.Count == 0
            ? 0d
            : rules.Average(rule => EstimateBalancedRangeAverage(GetRuleAmountBounds(rule)));
    }

    private static double EstimateBalancedRangeAverage(AmountBounds bounds)
    {
        return RoundAmountToUnit(bounds.Min + ((bounds.Max - bounds.Min) * 0.5d), bounds.Unit);
    }
        ...
        Random random)
    {
        ...
        var weekendSlices = allSlices
            .Where(rule => rule.TradeWeekend != false)
            .ToArray();
        var existingCountsByDay = new Dictionary<DateTime, int>();
        foreach (var record in records)
        {
            if (!record.AccountTime.HasValue
                || (record.TradeMoney ?? 0) >= -0.009d
                || Math.Abs(record.TradeMoney ?? 0) < 10d - 0.009d
                || Math.Abs(record.TradeMoney ?? 0) >= 30d - 0.009d
                || !IsReferenceGeneratedRecord(record))
            {
                continue;
            }

            var recordDate = record.AccountTime.Value.Date;
            existingCountsByDay[recordDate] = existingCountsByDay.GetValueOrDefault(recordDate) + 1;
        }
        ...
        for (var date = monthStart.Date; date <= normalizedEnd.Date; date = date.AddDays(1))
        {
            ...
            IReadOnlyList<GenerateReferenceRule> slices = date.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday
                ? weekendSlices
                : allSlices;
            if (slices.Count == 0)
            {
            ...
            if (existingCountsByDay.GetValueOrDefault(date) > 0)
            {
                continue;
            }
        ...
        out double amount)
    {
        ...
        amount = ClampAmountToBounds(raw, new AmountBounds(bounds.Min, max, bounds.Unit, bounds.Precision));
        amount = MoveAmountAwayFromEdges(amount, new AmountBounds(bounds.Min, max, bounds.Unit, bounds.Precision), random);
        ...
        out double amount)
    {
        ...
        amount = MakeAmountDistinctWithinBounds(
            amount,
            new AmountBounds(bounds.Min, max, bounds.Unit, bounds.Precision),
            existingRecords,
            isIncome,
            random);
        return amount >= bounds.Min - 0.009d && amount <= max + 0.009d;
        ...
        out double amount)
    {
        ...
        var effectiveBounds = new AmountBounds(bounds.Min, max, bounds.Unit, bounds.Precision);
        IReadOnlyList<double> values = CreateRuleAmountPoolValues(effectiveBounds);
        if (values.Count == 0)
        {
            return false;
        }
        ...
        var sign = isIncome ? 1 : -1;
        var usage = amountUsage?.GetUsage(
                sourceKey,
                sign,
                effectiveBounds,
                max >= bounds.Max - 0.009d)
            ?? existingRecords
                .Where(item => GetAmountSign(item.TradeMoney ?? 0) == sign)
                ...
                .GroupBy(item => item)
                .ToDictionary(group => group.Key, group => group.Count());
        var tailUsage = effectiveBounds.Precision is > 0
            ? amountUsage?.GetTailUsage(sourceKey, sign)
                ?? existingRecords
                    .Where(item => GetAmountSign(item.TradeMoney ?? 0) == sign)
                    .Where(item => string.Equals(GetGeneratedAmountDistributionKey(item), sourceKey, StringComparison.Ordinal))
                    .Select(item => GetConfiguredPrecisionTail(
                        Math.Abs(item.TradeMoney ?? 0),
                        effectiveBounds.Precision.Value))
                    .GroupBy(item => item)
                    .ToDictionary(group => group.Key, group => group.Count())
            : null;
        var globalTailUsage = effectiveBounds.Precision is > 0
            ? amountUsage?.GetGlobalTailUsage(sign, effectiveBounds.Precision.Value)
                ?? existingRecords
                    .Where(item => GetAmountSign(item.TradeMoney ?? 0) == sign)
                    .Where(item => GetRecordAmountBounds(item).Precision == effectiveBounds.Precision)
                    .Select(item => GetConfiguredPrecisionTail(
                        Math.Abs(item.TradeMoney ?? 0),
                        effectiveBounds.Precision.Value))
                    .GroupBy(item => item)
        ...
        AmountSegment? selectedDistributionSegment = null;
        var distributionSegments = CreateRuleAmountDistributionSegments(effectiveBounds);
        if (distributionSegments.Count > 1 && values.Count > 1)
        {
            var segmentUsage = new int[distributionSegments.Count];
            var segmentHasValues = new bool[distributionSegments.Count];
            foreach (var value in values)
            {
                var segmentIndex = GetAmountSegmentIndex(value, distributionSegments);
                if (segmentIndex >= 0)
                {
                    segmentHasValues[segmentIndex] = true;
                }
            }

            foreach (var item in usage)
            {
                var segmentIndex = GetAmountSegmentIndex(item.Key, distributionSegments);
                if (segmentIndex >= 0)
                {
                    segmentUsage[segmentIndex] += item.Value;
                }
            }

            var bestSegmentUsage = int.MaxValue;
            var tiedSegments = 0;
            foreach (var segment in distributionSegments)
            {
                if (!segmentHasValues[segment.Index])
                {
                    continue;
                }

                var currentUsage = segmentUsage[segment.Index];
                if (currentUsage < bestSegmentUsage)
                {
                    selectedDistributionSegment = segment;
                    bestSegmentUsage = currentUsage;
                    tiedSegments = 1;
                }
                else if (currentUsage == bestSegmentUsage)
                {
                    tiedSegments++;
                    if (random.Next(tiedSegments) == 0)
                    {
                        selectedDistributionSegment = segment;
                    }
                }
        ...
        if (ShouldPreferFractionalAmount(effectiveBounds)
            && effectiveBounds.Precision is not > 0
            && values.Count > 1)
        {
        ...
        var selectedAmount = values[0];
        var bestProtectedTailRank = int.MaxValue;
        var bestGlobalTailUsage = int.MaxValue;
        var bestSourceTailUsage = int.MaxValue;
        var bestUsage = int.MaxValue;
        var bestEdge = int.MaxValue;
        var tiedValues = 0;
        foreach (var value in values)
        {
            if (selectedDistributionSegment.HasValue
                && (value < selectedDistributionSegment.Value.Min - 0.009d
                    || value > selectedDistributionSegment.Value.Max + 0.009d))
            {
                continue;
            }

            var tail = effectiveBounds.Precision is > 0
                ? GetConfiguredPrecisionTail(value, effectiveBounds.Precision.Value)
                : -1;
            var valueGlobalTailUsage = tail >= 0 ? globalTailUsage?.GetValueOrDefault(tail) ?? 0 : 0;
            var valueSourceTailUsage = tail >= 0 ? tailUsage?.GetValueOrDefault(tail) ?? 0 : 0;
            var protectedTailRank = effectiveBounds.Precision == 2
                && IsProtectedNinetiesPrecisionTail(tail)
                && valueGlobalTailUsage > 0
                    ? tail == 99 ? 2 : 1
                    : 0;
            var isEdge = IsReferenceEdgeValue(value, effectiveBounds.Min, effectiveBounds.Max);
            var valueUsage = usage.GetValueOrDefault(value) + (isEdge ? 1 : 0);
            var edgeRank = isEdge ? 1 : 0;
            if (protectedTailRank < bestProtectedTailRank
                || protectedTailRank == bestProtectedTailRank && valueGlobalTailUsage < bestGlobalTailUsage
                || protectedTailRank == bestProtectedTailRank && valueGlobalTailUsage == bestGlobalTailUsage && valueSourceTailUsage < bestSourceTailUsage
                || protectedTailRank == bestProtectedTailRank && valueGlobalTailUsage == bestGlobalTailUsage && valueSourceTailUsage == bestSourceTailUsage && valueUsage < bestUsage
                || protectedTailRank == bestProtectedTailRank && valueGlobalTailUsage == bestGlobalTailUsage && valueSourceTailUsage == bestSourceTailUsage && valueUsage == bestUsage && edgeRank < bestEdge)
            {
                selectedAmount = value;
                bestProtectedTailRank = protectedTailRank;
                bestGlobalTailUsage = valueGlobalTailUsage;
                bestSourceTailUsage = valueSourceTailUsage;
                bestUsage = valueUsage;
                bestEdge = edgeRank;
                tiedValues = 1;
            }
            else if (protectedTailRank == bestProtectedTailRank
                && valueGlobalTailUsage == bestGlobalTailUsage
                && valueSourceTailUsage == bestSourceTailUsage
                && valueUsage == bestUsage
                && edgeRank == bestEdge)
            {
                tiedValues++;
                if (random.Next(tiedValues) == 0)
                {
                    selectedAmount = value;
                }
            }
    ...
    private static string GetRuleAmountDistributionKey(FlowRuleBase rule)
    {
        ...
        return CreateAmountDistributionKey(
            sourceKind,
            rule.Index.ToString(CultureInfo.InvariantCulture),
            GetRuleAmountBounds(rule));
    }

    private static string CreateAmountDistributionKey(
        string sourceKind,
        string sourceIndex,
        AmountBounds bounds)
    {
        return string.Create(
            CultureInfo.InvariantCulture,
            $"{sourceKind}:{sourceIndex}:{bounds.Min:0.########}:{bounds.Max:0.########}:{bounds.Unit:0.########}");
    }

    private static IReadOnlyList<double> CreateRuleAmountPoolValues(AmountBounds bounds)
    {
        if (RuleAmountPoolValues.TryGetValue(bounds, out var cached))
        {
            return cached;
        }

        var values = BuildRuleAmountPoolValues(bounds);
        return RuleAmountPoolValues.Count < MaxRuleAmountPoolCacheEntries
            ? RuleAmountPoolValues.GetOrAdd(bounds, values)
            : values;
    }

    private static double[] BuildRuleAmountPoolValues(AmountBounds bounds)
    {
        ...
        if (unit < 1d - 0.0000001d && bounds.Precision is not > 0)
        {
        ...
        if (bounds.Precision is > 0)
        {
            var precision = Math.Min(bounds.Precision.Value, 2);
            var scale = precision == 1 ? 10 : 100;
            var midpointWhole = (long)Math.Floor((bounds.Min + bounds.Max) / 2d);
            for (var tail = 1; tail < scale; tail++)
            {
                if (tail % 10 == 0)
                {
                    continue;
                ...
                var candidate = midpointWhole + tail / (double)scale;
                if (candidate < bounds.Min - 0.009d)
                {
                    candidate = Math.Floor(bounds.Min) + tail / (double)scale;
                }

                if (candidate > bounds.Max + 0.009d)
                {
                    candidate = Math.Floor(bounds.Max) + tail / (double)scale;
                }

                if (candidate >= bounds.Min - 0.009d && candidate <= bounds.Max + 0.009d)
                {
                    result.Add(ClampAmountToBounds(candidate, bounds));
                }
            }
        }

        return result
            .Where(item => HasRequiredNonZeroPrecisionDigit(item, bounds))
            .Distinct()
            .OrderBy(item => item)
            .ToArray();
    }
    ...
    private static List<AmountSegment> CreateRuleAmountDistributionSegments(AmountBounds bounds)
    {
        var unit = Math.Max(0.01d, bounds.Unit);
        var totalSteps = Math.Floor((bounds.Max - bounds.Min + 0.0000001d) / unit) + 1d;
        var segmentCount = (int)Math.Clamp(Math.Min(10d, totalSteps), 1d, 10d);
        if (segmentCount <= 1)
        {
            return [new AmountSegment(bounds.Min, bounds.Max, 0)];
        }
        ...
        for (var index = 0; index < segmentCount; index++)
        {
            ...
            var minimum = CeilAmountToUnit(bounds.Min + (startStep * unit), unit);
            var maximum = FloorAmountToUnit(bounds.Min + (Math.Max(startStep, endStep) * unit), unit);
            var segmentBounds = new AmountBounds(minimum, maximum, unit, bounds.Precision);
            if (RequiresNonZeroPrecisionDigit(segmentBounds))
            {
                var validMinimum = FindValidAmountAtOrAbove(minimum, maximum, segmentBounds);
                var validMaximum = FindValidAmountAtOrBelow(maximum, minimum, segmentBounds);
                if (!validMinimum.HasValue || !validMaximum.HasValue || validMinimum.Value > validMaximum.Value + 0.009d)
                {
                    continue;
                }
            ...
            result.Add(new AmountSegment(RoundMoney(minimum), RoundMoney(maximum), result.Count));
        }

        return result.Count == 0
            ? [new AmountSegment(bounds.Min, bounds.Max, 0)]
            : result;
    }
    ...
    {
        if (record.ExtraFields.TryGetValue(AmountDistributionKeyField, out var cachedKey)
            && !string.IsNullOrWhiteSpace(cachedKey))
        {
            return cachedKey;
        }
        ...
        {
            return CreateAmountDistributionKey(sourceKind, sourceIndex, GetRecordAmountBounds(record));
        }
        ...
        Random random)
    {
        ...
        foreach (var group in groups)
        {
            ...
            var desiredTotal = RoundAmountToUnit(
                EstimateBalancedRangeAverage(bounds) * groupRecords.Count,
        ...
        Random random)
    {
                     ...
                     .GroupBy(item => item.segment.Index))
        {
            ...
            for (var position = 0; position < positions.Count; position++)
            {
                ...
                var segmentBounds = new AmountBounds(segment.Min, segment.Max, unit, bounds.Precision);
        ...
        Random random)
    {
        ...
        var usedAmounts = existingRecords
            ...
            .ToHashSet();
        var preferFractional = bounds.Precision is > 0;
        ...
        NativeScheduleState scheduleState)
    {
        ...
        var bestNeighborSame = int.MaxValue;
        ...
        for (var index = 0; index < candidates.Count; index++)
        {
            ...
            var same = scheduleState.GetSameDirectionCount(date, isIncome);
            var neighborSame = scheduleState.GetNeighborSameDirectionCount(date, isIncome);
            var total = scheduleState.GetTotalCount(date);
            var distance = Math.Abs(index - desired);
            var comparison = same.CompareTo(bestSame);
            if (comparison == 0)
            {
                comparison = neighborSame.CompareTo(bestNeighborSame);
            }

            if (comparison == 0)
            {
                comparison = total.CompareTo(bestTotal);
            }

            if (comparison == 0)
            {
                comparison = distance.CompareTo(bestDistance);
            }
        ...
        Random random)
    {
        ...
        EnforceMonthlyTargetCap(incomeTargets, targetIncome, monthlyIncomeCap, random);
    ...
    private static double CalculateMonthlyIncomeCap(double targetIncome, int monthCount)
    {
        if (monthCount <= 0)
        {
            return 0;
        ...
        return RoundMoney(Math.Max(0, targetIncome) / monthCount * MonthlyIncomeCapMultiplier);
    }

    private static int FindMonthlyPlanIndex(
        IReadOnlyList<(DateTime Start, DateTime End)> months,
        DateTime accountTime)
    {
        for (var index = 0; index < months.Count; index++)
        {
            if (accountTime >= months[index].Start && accountTime <= months[index].End)
            {
                return index;
            }
        }
    ...
    private static void EnforceMonthlyTargetCap(
        double[] targets,
        double targetTotal,
        double monthlyCap,
        Random random)
    {
        ...
        targetTotal = RoundMoney(Math.Max(0, targetTotal));
        monthlyCap = RoundMoney(Math.Max(0, monthlyCap));
        var excess = 0d;
        for (var index = 0; index < targets.Length; index++)
        {
            targets[index] = RoundMoney(Math.Max(0, targets[index]));
            if (targets[index] <= monthlyCap + 0.009d)
            {
                continue;
            }

            excess = RoundMoney(excess + targets[index] - monthlyCap);
            targets[index] = monthlyCap;
        }

        while (excess > 0.009d)
        {
            var receivers = Enumerable.Range(0, targets.Length)
                .Where(index => targets[index] < monthlyCap - 0.009d)
                .OrderBy(index => targets[index] / Math.Max(0.01d, monthlyCap))
                .ThenBy(_ => random.Next())
                .ToList();
            ...
            var progressed = false;
            for (var position = 0; position < receivers.Count && excess > 0.009d; position++)
            {
                var index = receivers[position];
                var room = RoundMoney(monthlyCap - targets[index]);
                var slotsLeft = receivers.Count - position;
                var share = RoundMoney(excess / Math.Max(1, slotsLeft));
                var addition = RoundMoney(Math.Min(room, Math.Max(0.01d, share)));
                if (addition <= 0.009d)
                {
                    continue;
                }

                targets[index] = RoundMoney(targets[index] + addition);
                excess = RoundMoney(excess - addition);
                progressed = true;
            }
        ...
        var difference = RoundMoney(targetTotal - targets.Sum());
        if (difference > 0.009d)
        {
            foreach (var index in Enumerable.Range(0, targets.Length)
                         .Where(index => targets[index] < monthlyCap - 0.009d)
                         .OrderBy(index => targets[index])
                         .ThenBy(_ => random.Next()))
            {
                var addition = RoundMoney(Math.Min(difference, monthlyCap - targets[index]));
                targets[index] = RoundMoney(targets[index] + addition);
                difference = RoundMoney(difference - addition);
                if (difference <= 0.009d)
                {
                    break;
                }
        ...
        else if (difference < -0.009d)
        {
            var reductionLeft = Math.Abs(difference);
            foreach (var index in Enumerable.Range(0, targets.Length)
                         .OrderByDescending(index => targets[index])
                         .ThenBy(_ => random.Next()))
            {
                var reduction = RoundMoney(Math.Min(reductionLeft, targets[index]));
                targets[index] = RoundMoney(targets[index] - reduction);
                reductionLeft = RoundMoney(reductionLeft - reduction);
                if (reductionLeft <= 0.009d)
                {
                    break;
                }
            }
    ...
    private sealed class BalanceRangeIndex
    {
        private readonly DateTime[] accountTimes;
        private readonly Dictionary<FlowRecord, int> recordIndexes;
        private readonly double[] minimumTree;
        private readonly double[] lazyTree;
        private readonly int treeLeafCount;
        private readonly double openingBalance;

        private BalanceRangeIndex(
            DateTime[] accountTimes,
            Dictionary<FlowRecord, int> recordIndexes,
            double[] balances,
            double openingBalance)
        {
            ...
            treeLeafCount = 1;
            while (treeLeafCount < balances.Length)
            {
                treeLeafCount <<= 1;
            }

            minimumTree = Enumerable.Repeat(double.PositiveInfinity, treeLeafCount * 2).ToArray();
            lazyTree = new double[treeLeafCount * 2];
            for (var index = 0; index < balances.Length; index++)
            {
                minimumTree[treeLeafCount + index] = balances[index];
            }

            for (var index = treeLeafCount - 1; index > 0; index--)
            {
                minimumTree[index] = Math.Min(minimumTree[index * 2], minimumTree[(index * 2) + 1]);
            }
        }

        public double GlobalMinimum => Math.Min(openingBalance, minimumTree[1]);

        public static BalanceRangeIndex Create(IReadOnlyList<FlowRecord> records, double openingBalance)
        {
            var accountTimes = new List<DateTime>(records.Count);
            var balances = new List<double>(records.Count);
            var recordIndexes = new Dictionary<FlowRecord, int>(ReferenceEqualityComparer.Instance);
            foreach (var record in records)
            {
                var balance = record.Balance ?? openingBalance;
                if (!record.AccountTime.HasValue)
                {
                    continue;
                }

                recordIndexes[record] = accountTimes.Count;
                accountTimes.Add(record.AccountTime.Value);
                balances.Add(balance);
            }

            return new BalanceRangeIndex(
                accountTimes.ToArray(),
                recordIndexes,
                balances.ToArray(),
                openingBalance);
        }

        public double GetMinimum(DateTime startInclusive, DateTime endExclusive, double defaultValue)
        {
            var startIndex = LowerBound(startInclusive);
            var endIndex = LowerBound(endExclusive);
            if (startIndex >= endIndex)
            {
                return defaultValue;
            }

            var minimum = QueryMinimum(1, 0, treeLeafCount, startIndex, endIndex);
            return double.IsPositiveInfinity(minimum) ? defaultValue : minimum;
        }

        public double GetBalanceBefore(DateTime accountTime, double defaultValue)
        {
            var index = LowerBound(accountTime) - 1;
            if (index < 0)
            {
                return defaultValue;
            }

            return QueryMinimum(1, 0, treeLeafCount, index, index + 1);
        }

        public double GetSuffixMinimum(DateTime accountTime, double defaultValue)
        {
            var startIndex = LowerBound(accountTime);
            if (startIndex >= accountTimes.Length)
            {
                return defaultValue;
            }

            return QueryMinimum(1, 0, treeLeafCount, startIndex, accountTimes.Length);
        }

        public double GetSuffixMinimum(FlowRecord record, double defaultValue)
        {
            return recordIndexes.TryGetValue(record, out var startIndex)
                ? QueryMinimum(1, 0, treeLeafCount, startIndex, accountTimes.Length)
                : defaultValue;
        }

        public void AddToSuffix(DateTime accountTime, double amount)
        {
            AddRange(LowerBound(accountTime), accountTimes.Length, amount);
        }

        public void AddToSuffix(FlowRecord record, double amount)
        {
            if (recordIndexes.TryGetValue(record, out var startIndex))
            {
                AddRange(startIndex, accountTimes.Length, amount);
            }
        }

        public int GetInsertionIndex(DateTime accountTime)
        {
            return LowerBound(accountTime);
        }

        public void AddToRange(DateTime startInclusive, DateTime endExclusive, double amount)
        {
            AddRange(LowerBound(startInclusive), LowerBound(endExclusive), amount);
        }

        private void AddRange(int startIndex, int endIndex, double amount)
        {
            if (startIndex >= endIndex || Math.Abs(amount) <= 0.0000001d)
            {
                return;
            }

            AddRange(1, 0, treeLeafCount, startIndex, endIndex, amount);
        }

        private void AddRange(int node, int left, int right, int queryLeft, int queryRight, double amount)
        {
            if (queryLeft <= left && right <= queryRight)
            {
                minimumTree[node] += amount;
                lazyTree[node] += amount;
                return;
            }
            ...
            AddRange(node * 2, left, middle, queryLeft, queryRight, amount);
            AddRange((node * 2) + 1, middle, right, queryLeft, queryRight, amount);
            minimumTree[node] = Math.Min(minimumTree[node * 2], minimumTree[(node * 2) + 1]);
        }

        private double QueryMinimum(int node, int left, int right, int queryLeft, int queryRight)
        {
            if (queryLeft <= left && right <= queryRight)
            {
                return minimumTree[node];
            }
            ...
            var middle = left + ((right - left) / 2);
            return Math.Min(
                QueryMinimum(node * 2, left, middle, queryLeft, queryRight),
                QueryMinimum((node * 2) + 1, middle, right, queryLeft, queryRight));
        }

        private void PushLazy(int node)
        {
            var amount = lazyTree[node];
            if (Math.Abs(amount) <= 0.0000001d || node >= treeLeafCount)
            {
                return;
            }

            minimumTree[node * 2] += amount;
            minimumTree[(node * 2) + 1] += amount;
            lazyTree[node * 2] += amount;
            lazyTree[(node * 2) + 1] += amount;
            lazyTree[node] = 0;
        }

        private int LowerBound(DateTime value)
        {
            var low = 0;
            var high = accountTimes.Length;
            while (low < high)
            {
                var middle = low + ((high - low) / 2);
                if (accountTimes[middle] < value)
                {
                    low = middle + 1;
                }
                else
        ...
        Random random)
    {
        ...
        for (var index = 0; index < monthCount; index++)
        {
            ...
            {
                targets[index] = RoundMoney(Math.Min(finalBalanceTarget, available));
                break;
            }
    ...
    private static double[] CreateMonthlyExpenseTargetsFromClosingBalances(
        IReadOnlyList<double> incomeTargets,
        IReadOnlyList<double> closingBalanceTargets,
        double openingBalance,
        double targetExpense)
    {
        var monthCount = Math.Min(incomeTargets.Count, closingBalanceTargets.Count);
        var targets = new double[monthCount];
        var previousClosing = RoundMoney(Math.Max(0, openingBalance));
        for (var index = 0; index < monthCount; index++)
        {
            var available = RoundMoney(previousClosing + Math.Max(0, incomeTargets[index]));
            var closing = RoundMoney(Math.Clamp(closingBalanceTargets[index], 0, available));
            targets[index] = RoundMoney(Math.Max(0, available - closing));
            previousClosing = closing;
        }

        var difference = RoundMoney(targetExpense - targets.Sum());
        if (monthCount > 0 && Math.Abs(difference) > 0.009d)
        {
            targets[^1] = RoundMoney(Math.Max(0, targets[^1] + difference));
        }
        ...
        double amount)
    {
        ...
        var sign = amountSign > 0 ? 1 : -1;
        var template = RuleRecordTemplates.GetOrAdd(
            (rule, request.Bank, request.BankUser.Id, sign),
            _ => CreateRecordTemplate(request, rule, ruleColumns, amountBounds, sign));
        ...
        record.IncomeFlag = amount >= 0 ? "C" : "D";
        RecordAmountBounds.Add(record, new CachedAmountBounds(amountBounds));

        ApplyAutoGeneratedSystemFields(request, record);
        return record;
    }

    private static FlowRecord CreateRecordTemplate(
        FlowAutoGenerationRequest request,
        FlowRuleBase rule,
        IReadOnlyList<ColumnDefinition> ruleColumns,
        ...
        int sign)
    {
        var record = CreateBaseRecord(request, DateTime.MinValue, sign);
        ...
        record.ExtraFields[AmountPrecisionField] = (rule.FloutLength ?? 0).ToString(CultureInfo.InvariantCulture);
        ...
        record.ExtraFields[AmountDistributionKeyField] = GetRuleAmountDistributionKey(rule);
    ...
    private static void AlignMonthlyExpensesToBalanceWaves(
        double[] expenseTargets,
        IReadOnlyList<double> incomeTargets,
        double targetExpense,
        Random random)
    {
        var monthCount = Math.Min(expenseTargets.Length, incomeTargets.Count);
        var totalIncome = RoundMoney(incomeTargets.Take(monthCount).Sum(item => Math.Max(0, item)));
        targetExpense = RoundMoney(Math.Clamp(targetExpense, 0, totalIncome));
        if (monthCount < 3 || totalIncome <= 0.009d || targetExpense <= 0.009d)
        {
            return;
        }

        var finalCarry = RoundMoney(Math.Max(0, totalIncome - targetExpense));
        var averageIncome = totalIncome / monthCount;
        var carry = 0d;
        var buildLength = random.Next(1, 4);
        var buildPosition = 0;
        for (var index = 0; index < monthCount; index++)
        {
            var income = Math.Max(0, incomeTargets[index]);
            var available = RoundMoney(carry + income);
            double desiredCarry;
            if (index == monthCount - 1)
            {
                desiredCarry = Math.Min(available, finalCarry);
            }
            else if (buildPosition < buildLength)
            {
                // Retain most of the current month's income for one to three months so
                // the balance visibly climbs before the following drain month.
                var retainedIncome = income * (0.62d + (random.NextDouble() * 0.28d));
                desiredCarry = Math.Min(available, Math.Max(finalCarry, carry + retainedIncome));
                buildPosition++;
            }
            else
            {
                // Drain the accumulated carry close to zero, then begin a new independent
                // build cycle. The final configured carry remains the hard lower target.
                var lowTarget = averageIncome * (0.002d + (random.NextDouble() * 0.010d));
                desiredCarry = Math.Min(available, Math.Max(finalCarry, lowTarget));
                buildLength = random.Next(1, 4);
                buildPosition = 0;
            }

            expenseTargets[index] = RoundMoney(Math.Max(0, available - desiredCarry));
            carry = RoundMoney(desiredCarry);
        }

        NormalizeMonthlyTargetsInPlace(expenseTargets, targetExpense);
    }
    ...
    private static void RebalanceSignedRecords(IReadOnlyList<FlowRecord> signedRecords, double targetTotal)
    {
        ...
        {
            var precision = GetRecordAmountBounds(signedRecords[index]).Precision;
            amounts[index] = ClampAmountToBounds(
                amounts[index],
                new AmountBounds(mins[index], maxs[index], units[index], precision));
        }
        ...
        double targetTotal)
    {
        ...
        var active = new bool[signedRecords.Count];
        var activeCount = activeIndexes.Count;
        var activeMinTotal = 0d;
        var activeMaxTotal = 0d;
        var currentTotal = 0d;
        var monthlyIncomeCounts = new Dictionary<int, int>();
        foreach (var index in activeIndexes)
        {
            active[index] = true;
            activeMinTotal += mins[index];
            activeMaxTotal += maxs[index];
            currentTotal += amounts[index];
            var record = signedRecords[index];
            if ((record.TradeMoney ?? 0) > 0.009d && record.AccountTime.HasValue)
            {
                var month = record.AccountTime.Value;
                var monthKey = (month.Year * 100) + month.Month;
                monthlyIncomeCounts[monthKey] = monthlyIncomeCounts.GetValueOrDefault(monthKey) + 1;
            }
        }

        activeMinTotal = RoundMoney(activeMinTotal);
        activeMaxTotal = RoundMoney(activeMaxTotal);
        currentTotal = RoundMoney(currentTotal);
        var removalOrder = activeIndexes
        ...
        bool CanRemove(int index)
        {
            ...
            var month = record.AccountTime.Value;
            return monthlyIncomeCounts.GetValueOrDefault((month.Year * 100) + month.Month) > 3;
        }
        ...
        void Remove(int index)
        {
            active[index] = false;
            activeCount--;
            activeMinTotal = RoundMoney(activeMinTotal - mins[index]);
            activeMaxTotal = RoundMoney(activeMaxTotal - maxs[index]);
            currentTotal = RoundMoney(currentTotal - amounts[index]);
            var record = signedRecords[index];
            if ((record.TradeMoney ?? 0) > 0.009d && record.AccountTime.HasValue)
            {
                var month = record.AccountTime.Value;
                var monthKey = (month.Year * 100) + month.Month;
                monthlyIncomeCounts[monthKey] = Math.Max(
                    0,
                    monthlyIncomeCounts.GetValueOrDefault(monthKey) - 1);
            }
        ...
        var removalCursor = 0;
        while (activeCount > 0 && activeMinTotal > targetTotal + 0.009d)
        {
            while (removalCursor < removalOrder.Length && !CanRemove(removalOrder[removalCursor]))
            {
                removalCursor++;
            }

            if (removalCursor >= removalOrder.Length)
            {
                break;
            }

            Remove(removalOrder[removalCursor++]);
        }
        ...
        for (var attempt = 0; attempt < signedRecords.Count; attempt++)
        {
            ...
            foreach (var index in removalOrder)
            {
                if (!CanRemove(index))
                {
                    continue;
                ...
                if (activeCount > 1)
                {
                    var nextMinTotal = RoundMoney(activeMinTotal - mins[index]);
                    var nextMaxTotal = RoundMoney(activeMaxTotal - maxs[index]);
                ...
                if (amount <= excess * 1.15d
                    || excess > targetTotal * 0.25d
                    || nextExcess < Math.Abs(excess))
                {
        ...
        activeIndexes.RemoveAll(index => !active[index]);
    }
    ...
    private static bool EnsureDistinctSignedAmounts(List<FlowRecord> records, bool isIncome)
    {
        ...
        foreach (var record in signedRecords)
        {
            ...
            var bounds = GetRecordAmountBounds(record);
            var preferFractional = bounds.Precision is > 0;
            var requiresPrecisionCorrection = preferFractional
                && !HasRequiredNonZeroPrecisionDigit(amount, bounds);
            if (!usedAmounts.Contains(amount) && !requiresPrecisionCorrection)
            {
        ...
        out double distinctAmount)
    {
        ...
        void Consider(double candidate, bool allowWholeAmount)
        {
            ...
            if (!allowWholeAmount
                && (!HasRequiredNonZeroPrecisionDigit(candidate, bounds)
                    || HasUnnaturalFractionTail(candidate)))
            {
                return;
            }
        ...
        var preferNonWhole = preferFractional;
    ...
    private static int GetConfiguredPrecisionTail(double amount, int precision)
    {
        precision = Math.Clamp(precision, 1, 2);
        var scale = precision == 1 ? 10d : 100d;
        return (int)Math.Round(Math.Abs(RoundMoney(amount)) * scale, MidpointRounding.AwayFromZero)
            % (int)scale;
    }
        ...
        double targetAmount)
    {
        ...
        for (var index = 0; index < maxAttempts && remaining > 0.009d; index++)
        {
            ...
            var effectiveBounds = new AmountBounds(bounds.Min, Math.Min(bounds.Max, maxAcceptable), bounds.Unit, bounds.Precision);
        ...
        bool allowDropOptional)
    {
        ...
        var candidates = GetSignedRecords(records, isIncome)
            ...
            .ThenByDescending(item => item.AccountTime ?? DateTime.MinValue)
            .ToList();
        var monthlyIncomeCounts = isIncome && allowDropOptional
            ? BuildMonthlyIncomeCounts(candidates)
            : null;
        ...
        foreach (var record in candidates)
        {
            ...
            {
                var canDropRecord = !isIncome
                    || CanDropOptionalIncomeRecord(record, monthlyIncomeCounts!);
                ...
                {
                    ApplySignedAmount(record, sign, 0);
                    if (isIncome && record.AccountTime.HasValue)
                    {
                        var month = record.AccountTime.Value;
                        var monthKey = (month.Year * 100) + month.Month;
                        monthlyIncomeCounts![monthKey] = Math.Max(
    ...
    private static bool CanDropOptionalIncomeRecord(
        FlowRecord candidate,
        IReadOnlyDictionary<int, int> monthlyIncomeCounts)
    {
        ...
        Random random)
    {
        ...
        RebalanceVeryHighVolumeLowCapacityExpense(
            ...
            openingBalance,
            random);
        perfTrace.Mark("fast low capacity", records);
        EnforceActualMonthlyIncomeCap(records, request, monthlyPlan, openingBalance, random);
        perfTrace.Mark("fast monthly income cap", records);
        EnforceMonthlyClosingBalanceTargets(records, request, monthlyPlan, openingBalance, random);
        perfTrace.Mark("fast monthly closing targets", records);
        ApplyBalances(records, openingBalance);
        if (GetMinimumBalance(records, openingBalance) < -0.009d)
        {
            RepairVeryHighVolumeNegativeBalances(records, openingBalance);
        ...
        perfTrace.Mark("fast monthly nonnegative", records);
        EnforceMonthlyClosingBalanceCapByMovingFutureExpenses(
            records,
            request,
            monthlyPlan,
            openingBalance,
            random);
        perfTrace.Mark("fast monthly closing cap", records);
        RestoreFinalMonthlyClosingTarget(records, request, monthlyPlan, openingBalance, random);
        perfTrace.Mark("fast final monthly target", records);
        if (EnforceActualMonthlyIncomeCap(records, request, monthlyPlan, openingBalance, random)
            && GetMinimumBalance(records, openingBalance) < -0.009d)
        {
            RepairVeryHighVolumeNegativeBalances(records, openingBalance);
    ...
    private readonly record struct RulePropertyMapping(PropertyInfo Source, PropertyInfo? Target);

    private readonly record struct NormalizedColumnDefinition(string Name, string Field);

    private readonly record struct ColumnFieldMapping(string SourceField, string TargetField);

    private sealed class CachedAmountBounds(AmountBounds value)
    {
        public AmountBounds Value { get; } = value;
    }

    private readonly record struct AmountBounds(double Min, double Max, double Unit, int? Precision = null);
        ...
        IReadOnlyList<GenerateReferenceRule> rules)
    {
        ...
        public double Expected { get; } = rules.Average(rule => EstimateBalancedRangeAverage(GetRuleAmountBounds(rule)));
    }
    ...
    private static bool TryGetRuleAmountBounds(FlowRuleBase rule, out AmountBounds bounds)
    {
        ...
        var precision = rule.FloutLength ?? 0;
        var candidateBounds = new AmountBounds(roundedMin, roundedMax, unit, precision);
        if (RequiresNonZeroPrecisionDigit(candidateBounds))
        {
            var firstValid = FindValidAmountAtOrAbove(roundedMin, roundedMax, candidateBounds);
            var lastValid = FindValidAmountAtOrBelow(roundedMax, roundedMin, candidateBounds);
        ...
        bounds = new AmountBounds(RoundMoney(roundedMin), RoundMoney(roundedMax), unit, precision);
        return true;
    }
    ...
    {
        return RecordAmountBounds.GetValue(record, static item =>
        {
            var unit = GetRecordAmountUnit(item);
            var min = item.ExtraFields.TryGetValue(AmountMinField, out var minValue) && TryParseDouble(minValue, out var parsedMin)
                ? parsedMin
                : 0d;
            var max = item.ExtraFields.TryGetValue(AmountMaxField, out var maxValue) && TryParseDouble(maxValue, out var parsedMax)
                ? parsedMax
                : double.MaxValue;
            var precision = item.ExtraFields.TryGetValue(AmountPrecisionField, out var precisionValue)
                ? ParseInt(precisionValue)
                : null;
            ...
            return new CachedAmountBounds(new AmountBounds(min, max, unit, precision));
        }).Value;
    }
    ...
    private static double ClampAmountToBounds(double amount, AmountBounds bounds)
    {
        ...
        if (!HasRequiredNonZeroPrecisionDigit(bounded, bounds))
        {
            var lower = FindValidAmountAtOrBelow(bounded, bounds.Min, bounds);
            var upper = FindValidAmountAtOrAbove(bounded, bounds.Max, bounds);
    ...
    private static bool RequiresNonZeroPrecisionDigit(AmountBounds bounds)
    {
        return bounds.Precision is > 0 || bounds.Unit >= 10d - 0.0000001d;
    }

    private static bool HasRequiredNonZeroPrecisionDigit(double amount, AmountBounds bounds)
    {
        if (!RequiresNonZeroPrecisionDigit(bounds))
        {
            return true;
        }
        ...
        if (bounds.Precision is > 0)
        {
            var precision = Math.Min(bounds.Precision.Value, 2);
            var scale = precision == 1 ? 10d : 100d;
            var scaled = (long)Math.Round(Math.Abs(RoundMoney(amount)) * scale, MidpointRounding.AwayFromZero);
            return scaled % 10L != 0L;
        }

        var precisionPlaceValue = Math.Floor((Math.Abs(amount) + 0.0000001d) / bounds.Unit);
        var digit = (int)(precisionPlaceValue % 10d);
        return digit is >= 1 and <= 9;
    }

    private static double? FindValidAmountAtOrAbove(double start, double maximum, AmountBounds bounds)
    {
        var candidate = CeilAmountToUnit(start, bounds.Unit);
        for (var offset = 0; offset <= 10 && candidate <= maximum + 0.009d; offset++, candidate = RoundMoney(candidate + bounds.Unit))
        {
            if (HasRequiredNonZeroPrecisionDigit(candidate, bounds))
            {
                return candidate;
            }
        }
    ...
    private static double? FindValidAmountAtOrBelow(double start, double minimum, AmountBounds bounds)
    {
        var candidate = FloorAmountToUnit(start, bounds.Unit);
        for (var offset = 0; offset <= 10 && candidate >= minimum - 0.009d; offset++, candidate = RoundMoney(candidate - bounds.Unit))
        {
            if (HasRequiredNonZeroPrecisionDigit(candidate, bounds))
            {
                return candidate;
    ...
    {
        var mappings = RulePropertyMappings.GetOrAdd(rule.GetType(), static ruleType =>
        {
            var recordType = typeof(FlowRecord);
            return ruleType
                .GetProperties(BindingFlags.Instance | BindingFlags.Public)
                .Where(property => !IgnoredRuleProperties.Contains(property.Name)
                    && property.CanRead
                    && property.GetIndexParameters().Length == 0)
                .Select(property => new RulePropertyMapping(
                    property,
                    recordType.GetProperty(property.Name, BindingFlags.Instance | BindingFlags.Public)))
                .Where(mapping => mapping.Target is not null
                    && mapping.Target.CanWrite
                    && mapping.Target.GetIndexParameters().Length == 0)
                .ToArray();
        });
        foreach (var mapping in mappings)
        {
            var value = mapping.Source.GetValue(rule);
            if (value is null)
            {
                continue;
            }

            SetConvertedValue(record, mapping.Target!, value);
        }
    }
    ...
    private static void CopyByMatchingColumnNames(
        ...
        FlowRecord record)
    {
        var mappings = ColumnFieldMappings.GetOrAdd((ruleColumns, flowColumns), static key =>
        {
            var ruleColumnsByName = new Dictionary<string, string>(StringComparer.Ordinal);
            foreach (var column in GetNormalizedColumnDefinitions(key.Rule))
            {
                ruleColumnsByName.TryAdd(column.Name, column.Field);
            }

            var result = new List<ColumnFieldMapping>();
            foreach (var flowColumn in GetNormalizedColumnDefinitions(key.Flow))
            {
                if (ruleColumnsByName.TryGetValue(flowColumn.Name, out var sourceField))
                {
                    result.Add(new ColumnFieldMapping(sourceField, flowColumn.Field));
                }
            }
        ...
        foreach (var mapping in mappings)
        {
            var value = GetRuleValue(rule, mapping.SourceField);
            if (value is null || string.IsNullOrWhiteSpace(Convert.ToString(value, CultureInfo.CurrentCulture)))
            {
                continue;
            }

            SetRecordValue(record, mapping.TargetField, value);
        }
    }
        ...
        FlowRecord record)
    {
        ...
        {
            ApplyColumnValue(bank, record, FirstNonEmpty(GetRuleColumnText(rule, ruleColumns, "交易对方"), rule.OppositeUsername, record.OppositeUsername), "交易对方");
            ApplyColumnValue(bank, record, GetRuleColumnText(rule, ruleColumns, "对方户名"), "对方户名");
        }
        else
        {
    ...
    private static void ApplyColumnValue(Bank bank, FlowRecord record, string? value, params string[] columnNames)
    {
        ...
        foreach (var column in GetNormalizedColumnDefinitions(bank.FlowColumns))
        {
            if (!MatchesAnyColumnName(column.Name, columnNames)
                || !string.IsNullOrWhiteSpace(GetRecordStringValue(record, column.Field)))
            {
                continue;
            }
    ...
    {
        foreach (var column in GetNormalizedColumnDefinitions(ruleColumns))
        {
            if (!MatchesAnyColumnName(column.Name, columnNames))
            {
                continue;
    ...
    private static NormalizedColumnDefinition[] GetNormalizedColumnDefinitions(
        IReadOnlyList<ColumnDefinition> columns)
    {
        return NormalizedColumnDefinitions.GetOrAdd(columns, static source => source
            .Where(item => !string.IsNullOrWhiteSpace(item.Name) && !string.IsNullOrWhiteSpace(item.Field))
            .Select(item => new NormalizedColumnDefinition(NormalizeName(item.Name!), item.Field!))
            .ToArray());
    }

    private static bool MatchesAnyColumnName(string normalizedName, IReadOnlyList<string> columnNames)
    {
        foreach (var columnName in columnNames)
        {
            var normalizedAlias = NormalizedColumnAliases.GetOrAdd(columnName, static name => NormalizeName(name));
            if (string.Equals(normalizedName, normalizedAlias, StringComparison.Ordinal))
            {
                return true;
            }
        }
    ...
    private static void ApplyAutoGeneratedSystemFields(FlowAutoGenerationRequest request, FlowRecord record)
    {
        ...
        if (IsWechatQrCollectionRecord(request.Bank, record))
        {
            record.MerchantName = "/";
        }
        else
    ...
    {
        return BankFlowFieldPresence.GetOrAdd(
            (bank, field),
            static key => key.Bank.FlowColumns.Any(item => string.Equals(item.Field, key.Field, StringComparison.Ordinal)));
    }
    ...
    private static string GetRecordStringValue(FlowRecord record, string field)
    {
        ...
        if (!FlowRecordProperties.TryGetValue(field, out var property))
        {
            return record[field];
        }
    ...
    private static bool IsWechatQrCollectionRecord(Bank bank, FlowRecord record)
    {
        return bank.Name.Contains("微信", StringComparison.Ordinal)
            && BuildWechatScenarioText(record).Contains("二维码收款", StringComparison.Ordinal);
    }
    ...
    private static void SetRecordValue(FlowRecord record, string field, object value)
    {
        ...
        if (!FlowRecordProperties.TryGetValue(field, out var property) || !property.CanWrite)
        {
    ...
    {
        if (int.TryParse(
                Environment.GetEnvironmentVariable("SPEEDEMULATOR_FLOW_TEST_SEED"),
                NumberStyles.Integer,
                CultureInfo.InvariantCulture,
                out var testSeed))
        {
            return unchecked((testSeed * 397) ^ salt);
        }
    ...
    private static string RandomDigits(int seed, int length)
    {
        ...
        return string.Create(length, random, static (span, source) =>
        {
            ...
            {
                span[index] = (char)('0' + source.Next(0, 10));
            }
        });
    }
